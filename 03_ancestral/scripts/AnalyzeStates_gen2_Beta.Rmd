...
focal.source.brf<-c("South Africa")
...
# f.out<-"2023-07-06_beta_analysis/"
...
```{r}
# sum.Par.Roll.summary<-read.csv("2023-07-06_beta_analysis/sum.par.roll.summary.csv")
...
p1.line.ci<-sum.Par.Roll.summary %>%
  ...
  labs(x="Date of most recent common ancestor",y=paste0("Sublineages per week"), fill="Origin Location")+
  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0),breaks=seq(0,y.lim2[2]-1,y.jumps))+
...
## version with label
plot_grid(p1.line.ci.noleg, labels="B) Beta",label_x=-0.1)
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confint_noleg_B.png",sep=""),height=3,width=3.5)
...
```{r}
# sing.Par.Roll.summary<-read.csv("2023-07-06_beta_analysis/sing.par.roll.summary.csv")
...
y.lim2<-c(0,(max(sing.Par.Roll.summary$upper.CI,na.rm=T)))
...
p1.sing.line.ci<-sing.Par.Roll.summary %>%
  ...
  annotate(geom="rect",xmin = int.start, xmax = int.end,
             ymin = y.lim2[1], ymax =y.lim2[2], color="grey85",size=0,  fill="grey80")+
    annotate(geom="text",
           x = (as.Date(int.start)-int.duration/2),y=y.lim2[2]-0.25, 
           vjust=0.5, hjust=0,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
  geom_ribbon(aes(ymin=lower.CI,ymax=upper.CI, 
  ...
  # scaleDateFlex+
  scale_x_date(limits=as.Date(c("2020-11-01","2021-06-01")),
 ...
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
  ...
  coord_cartesian(ylim=y.lim2)
p1.sing.line.ci
ggsave(paste(f.out,"/Rolling-singleton-importRate_line_confintALL_beta.png",sep=""),height=3,width=3.5)
...
y.lim2<-c(0,(ceiling(max(focal.var.cases$avgVOC_cases))*1.2))
p.focal.var.cases<-ggplot(focal.var.cases)+
    annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim2[1], ymax =y.lim2[2]-50, color="grey85",size=0,  
...
p.focal.pred.sublin<-focal.intros.all %>%
  ...
  annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim[1], ymax =y.lim[2], color="grey85",size=0,  fill="grey80")+
  annotate(geom="text",x = (as.Date(int.start)+int.duration/2),y=y.lim[2]-0.05, vjust=1, hjust=0.4,
           label=int.descrip,fontface="italic",color="black",size=4.3)+
  labs(x="Date of most recent common ancestor",y=paste0("Sublineages per week"))+
  scale_x_date(date_labels = "%b %Y",
                            limits=c(fig.start.date,fig.end.date),
                            expand=c(0,0))+
  scale_y_continuous(expand=c(0,0),breaks=seq(0,2,1))+
  #observed
...
# lm.cases.intros.sing<-lm(data=focal.cases.intros.sing, intros_meansum7d.mean ~ 1)#use a constant model instead (just estimating the intercept)
sing.lm.cases.intros.sing<-summary(lm.cases.intros.sing) 
# sing.lm.cases.intros.sing<-summary(lm.cases.intros.sing) 
...
y.lim2<-c(0,(ceiling(max(can.var.cases$avgVOC_cases,na.rm=T))*2))
...
p.can.var.cases<-ggplot(can.var.cases)+
    ...
    annotate(geom="label",x =( as.Date(int.start)+int.duration/2),
             y=(y.lim2[2]/5)*4, vjust=1, hjust=0.5,
             label=can.var.total.cases.text, size=3)+
    pubTheme+
    geom_bar(aes(x=date, y=avgVOC_cases),fill=myvar,stat="identity",width=1)+
    scale_x_date(breaks=xdates, date_labels = "%b %Y",
                              limits=as.Date(c(start.date,"2021-09-01")),
                              expand=c(0,0))+
    scale_y_continuous(expand=c(0,0))+
    # GlobFillScale+
    theme(legend.position = "none")+
