...
# f.out<-"2023-07-06_gamma_analysis/"
...
scaleDateFlexLess<-scale_x_date(date_breaks = "1 month",date_labels = "%b %Y",expand=c(0,0),
                                limits=as.Date(c(fig.start.date,fig.end.date)))
...
#   myroot<-decimal_date(as.Date(first(sort(td$date))))
  ...
  can.sing.boots[[k]]<-mutate(can.sing.boots[[k]],tmrca.dt.half=
                                tmrca.dt+(((date.lsd.full-tmrca.dt)/10)*8),
                              .keep=T)
...
id.varsub<-str_which(aliases$lineage,focal.pango) 
...
if(int.yn==T){
  ...
  ## What proportion of sublins from focal before the ban (2)
  ...
  file=text.out,sep="\n",append=T) 
    ...
    ## What proportion of sublins from focal before the ban (1)
  sublin.preban.prop.one<-c()
  for (k in 1:n.B){
    x<-length(which(sum.boots[[k]]$tmrca.dt<as.Date(int.start.one) &
                      sum.boots[[k]]$Parent.Location %in%focal.source))
    y<-length(which(sum.boots[[k]]$tmrca.dt<as.Date(int.start.one)))
    prop<-x/y
    sublin.preban.prop.one<-c(sublin.preban.prop.one, prop) }
  cat(paste0('Proportion of sublins pre-intervention (1) from focal = ',
         mean.95ci.X(sublin.preban.prop.one,2)  ), 
  file=text.out,sep="\n",append=T) 
  ...
  for (k in 1:n.B){
    x<-length(which(sum.boots[[k]]$tmrca.dt>=as.Date(int.start.one) &
                    sum.boots[[k]]$tmrca.dt<as.Date(int.end.one)  ))
    sublin.duringban<-c(sublin.duringban,x)}
   cat(paste0('Mean (95%CI) # of sublins during intervention (1) = ',
        mean.95ci.X(sublin.duringban,0) ), 
    file=text.out,sep="\n",append=T) 
  
  #DURING BAN from focal.source (2)
  sublin.duringban1<-c()
  for (k in 1:n.B){
    x<-length(which(sum.boots[[k]]$tmrca.dt>=as.Date(int.start) &
                    sum.boots[[k]]$tmrca.dt<as.Date(int.end) &
   ...
   #proportion of sublin during ban from focal (2)
   ...
   cat(paste0('Proportion of sublins during-intervention (2) from focal = ',
         mean.95ci.X(sublin.durban.prop,2)  ), 
    file=text.out,sep="\n",append=T) 
   
   #proportion of sublin during ban from focal (1)
    sublin.durban.prop.one<-c()
    for (k in 1:n.B){
    x<-length(which(sum.boots[[k]]$tmrca.dt>=as.Date(int.start.one) &
                    sum.boots[[k]]$tmrca.dt<as.Date(int.end.one) &
                      sum.boots[[k]]$Parent.Location %in%focal.source))
    y<-length(which(sum.boots[[k]]$tmrca.dt>=as.Date(int.start.one) &
                    sum.boots[[k]]$tmrca.dt<as.Date(int.end.one) ))
    prop<-x/y
    sublin.durban.prop.one<-c(sublin.durban.prop.one, prop) 
    sublin.durban.prop.one<-na.omit(sublin.durban.prop.one)}
   cat(paste0('Proportion of sublins during-intervention (1) from focal = ',
         mean.95ci.X(sublin.durban.prop.one,2)  ), 
    file=text.out,sep="\n",append=T) 
  ...
  ###Fold change in proportion focal pre vs during (restriction 2)
  sublin.fc.prop<-c()
  for (k in 1:n.B){
  ...
  ###Fold change in proportion focal pre vs during restriction 1
  sublin.fc.prop.one<-c()
  for (k in 1:n.B){
    x<-sublin.preban.prop.one[k]
    y<-as.numeric(sublin.durban.prop.one[k])
    fc<-x/y #if pre=0.5 and during=0.1, 0.5/0.1 = 5-fold reduction 
    sublin.fc.prop.one<-c(sublin.fc.prop.one, fc) }
   sublin.fc.prop.one<-na.omit( sublin.fc.prop.one)
  cat(paste0('Fold reduction in proportion of sublins pre vs during-intervention 1 from focal = ',
       mean.95ci.X(sublin.fc.prop.one,2)  ), 
  file=text.out,sep="\n",append=T) 
...
if(mean(un.lins)>1){
  ...
  alluv.lin<-ggplot(sum.Par.Prov.L.summary.long,
    ...
    geom_text(stat = "stratum", size = 3.3,min.y=3,label.padding=unit(0.1, "lines")) +
  ...
  alluv.lin.n<-ggplot(sum.Par.Prov.L.summary.long,
    ...
    scale_color_manual(name = "geo",values = globalPalette.ch,na.value="grey60")+
    geom_text(stat = "stratum", size = 3.3,min.y=3,label.padding=unit(0.1, "lines")) +
    geom_text(stat="flow",aes(label=Lineage,color=Lineage),size=1.5,min.y=2,color="white",
              nudge_x=-0.206,hjust=1)+
    pubTheme+
    theme(legend.position = "none",
          axis.ticks.x = element_blank(),axis.text.x = element_text(hjust=c(0.5,0.5)),
...
if(mean(un.lins)>1 & int.yn==T){
  ...
  linwaves
  ggsave(paste(f.out,"/Alluvial.Sublin.AllPeriods.Parent.Node.Lin.png",sep=""),height=5,width=10,units="in")  
}
```
...
P3<-ggplot(data=sum.boots.binSize)+
  ...
  scale_y_continuous(limits=c(0,15),expand=c(0.02,0.01))
...
## BRAZIL ONLY
p1.line.ci.brazil<-sum.Par.Roll.summary %>%
  filter(Parent.Location=="Brazil") %>%
  ggplot(aes(x=tmrca.dt,y=intros_meansum7d.mean,group=Parent.Location))+
  pubThemeDate+
  theme(text=element_text(size=10,face="bold"),
        legend.position = "none",
  ...
  geom_ribbon(aes(ymin=lower.CI,ymax=upper.CI, 
                  fill=Parent.Location, alpha=focal))+
  geom_line(stat="identity",lwd=1,aes(color=Parent.Location),alpha=1)+
  GlobRegColScale+
  GlobRegFillScale+
  scale_alpha_manual(values=alphavals)+
  labs(x="Date of most recent common ancestor",y=paste0("Sublineages per week"), fill="Origin Location")+
 ...
 guides(fill = guide_legend(keywidth = 0.4,keyheight=0.4,
  ...
  coord_cartesian(ylim=y.lim2)
p1.line.ci.brazil
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confint_brazil.png",sep=""),height=3,width=3.5)
...
```{r}
# sum.Par.Roll.summary<-read.csv("2023-07-06_gamma_analysis/sum.par.roll.summary.csv")
...
p1.line.ci<-sum.Par.Roll.summary %>%
  ...
  theme(text=element_text(size=10,face="bold"),
   ...
   annotate(geom="rect",xmin = as.Date(int.start.one), xmax = as.Date(int.end.one),
             ymin = y.lim2[1], ymax =y.lim2[2],
             color="grey85",size=0,  fill="grey80",alpha=0.9)+
  annotate(geom="text",x = as.Date(int.start)-10,
...
#                             limits=as.Date(c("2020-12-01","2021-07-14")),
#                             expand=c(0,0))+  
  scale_y_continuous(expand=c(0,0),breaks=seq(0,y.max.singles+1,y.jumps))+
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
                              title.position = "top",title="Global origin",
                              legend.spacing=0,ncol=1,
                               # override.aes = list(alpha = c(1,0.3,0.3,0.3))
                            ),
        color="none",
        alpha="none")+
  coord_cartesian(ylim=y.lim2)
...
p1.line.ci.noleg<-sum.Par.Roll.summary %>%
  ...
  theme(text=element_text(size=10,face="bold"),
   ...
   annotate(geom="rect",xmin = as.Date(int.start.one), xmax = as.Date(int.end.one),
             ymin = y.lim2[1], ymax =y.lim2[2],
             color="grey85",size=0,  fill="grey80",alpha=0.9)+
  annotate(geom="text",x = as.Date(int.start)-20,
           y=y.lim2[2]-0.1, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
  geom_ribbon(aes(ymin=lower.CI,ymax=upper.CI, 
                  fill=Parent.Location, alpha=focal))+
 ...
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
         ...
         alpha="none", color="none")+
  coord_cartesian(ylim=y.lim2)+
  annotate(geom="text",x=as.Date("2021-02-15"),y=2.5, label="Brazil",
           color=globalPalette.ch[which(names(globalPalette.ch)=="Brazil")],size=4)+
  annotate(geom="text",x=as.Date("2021-06-01"),y=1.9, label="South America",
           color=globalPalette.ch[which(names(globalPalette.ch)=="South America")],alpha=0.8,size=2.5)+
  annotate(geom="text",x=as.Date("2021-08-10"),y=1.05,label="USA",
           color=globalPalette.ch[which(names(globalPalette.ch)=="USA")],alpha=0.8,size=2.5)
...
## version with label
plot_grid(p1.line.ci.noleg, labels="C) Gamma",label_x=-0.1)
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confint_noleg_C.png",sep=""),height=3,width=3.5)
...
if(int.yn==T){
   ...
   cat(paste0("Fold decrease sublin rate pre-int to 2-weeks after int 2 from ",focal.source, 
              ": ", mean.fc.2wk),    file=text.out,sep="\n",append=T)
   
   cat(paste0("Fold decrease sublin rate pre-int to 4-weeks after int 2 from ",focal.source, 
...
## FIRST INTERVENTION
if(int.yn==T){
   rate.pre.2<-sum.Par.Roll.summary %>% filter(Parent.Location%in%focal.source) %>% filter(tmrca.dt==int.start.one)
   
   rate.2wk.2<-sum.Par.Roll.summary %>% filter(Parent.Location%in%focal.source) %>% filter(tmrca.dt==(int.start.one+14))
   
    rate.4wk.2<-sum.Par.Roll.summary %>% filter(Parent.Location%in%focal.source) %>% filter(tmrca.dt==(int.start.one+28))
   
   cat(paste0("Weekly sublin importation rate pre-int.1 from ",focal.source, 
              " on ",int.start.one,": ", 
              mean.95ci.givenmsd(m=rate.pre.2$intros_meansum7d.mean,
                                    sd=rate.pre.2$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   
   cat(paste0("Weekly sublin importation rate 2-weeks post-int.1 from ",focal.source, 
              " on ",int.start.one+14, ": ", 
              mean.95ci.givenmsd(m=rate.2wk.2$intros_meansum7d.mean,
                                    sd=rate.2wk.2$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   ...
   for (k in 1:n.B){
     temp<-Parent.Location.sum.boots.full[[k]] %>% 
      dplyr::filter(Parent.Location%in%focal.source) %>%
      as.data.frame()
     temp.pre.2<-temp %>% filter(tmrca.dt==int.start.one)
     temp.2wk.2<-temp %>% filter(tmrca.dt==(int.start.one+14))
     temp.4wk.2<-temp %>% filter(tmrca.dt==(int.start.one+28))

     fc2wk.2<-temp.pre.2$intros_meansum7d/temp.2wk.2$intros_meansum7d #fold change 2wk/pre (ex: 10/1)
     fc2wk.focal.2<-c(fc2wk.focal.2, fc2wk.2)
     
     fc4wk.2<-temp.pre.2$intros_meansum7d/temp.4wk.2$intros_meansum7d #fold change 2wk
     fc4wk.focal.2<-c(fc4wk.focal.2, fc4wk.2)
   }
   fc2wk.focal.2<-fc2wk.focal.2[is.finite(fc2wk.focal.2)]
   fc4wk.focal.2<-fc4wk.focal.2[is.finite(fc4wk.focal.2)]
   
   mean.fc.2wk.2<-mean.95ci.X(fc2wk.focal.2,2)
   mean.fc.4wk.2<-mean.95ci.X(fc4wk.focal.2,2)
  

   cat(paste0("Fold decrease sublin rate pre-int to 2-weeks after int 1 from ",focal.source, 
              ": ", mean.fc.2wk.2),    file=text.out,sep="\n",append=T)
   
   cat(paste0("Fold decrease sublin rate pre-int to 4-weeks after int 1 from ",focal.source, 
              ": ", mean.fc.4wk.2),    file=text.out,sep="\n",append=T)
...
p.ridge<-ggplot(temp.df, aes(x=Date, y=Sublineage,fill=Lineage),alpha=0.7) + 
  ...
  scale_x_date(date_labels = "%b %Y",expand=c(0,0),limits=as.Date(c("2020-10-01","2021-10-01")))+
...
cat(paste0("Percent singletons of all intros = ",
           mean.95ci.X((sing.tot/tot.tot*100),1)),
    file=text.out,sep="\n",append=t)

#what percent did the few large intros account for?
cat(paste0("Percent intros with >99 desc = ",
          mean.95ci.X( hund/tot.tot*100, 1)),
    file=text.out,sep="\n",append=T)

cat(paste0("Percent intros with >=500 desc = ",
...
p1.sing<-sing.Par.Roll.summary %>%
  ggplot(aes(x=tmrca.dt.half,y=intros_meansum7d.mean,group=par.state,fill=par.state))+
   ...
   annotate(geom="rect",xmin = as.Date(int.start.one), xmax = as.Date(int.end.one),
             ...
             color="grey85",size=0,  fill="grey80")+
    annotate(geom="text",x =( as.Date(int.start.one)+(int.end-int.start.one)/2),
             y=(y.lim[2]/100)*99, vjust=1, hjust=0.5,
             label=int.descrip,fontface="italic",color="grey25",size=3)+
  geom_density(stat="identity", position="stack",lwd=0,alpha=0.9)+
  CountryFillScale+
  pubThemeDate+
...
```{r}
sing.Par.Roll.summary<-read.csv("2023-07-06_gamma_analysis/sing.par.roll.summary.csv")
...
p1.sing.line.ci<-sing.Par.Roll.summary %>%
  ...
  theme(text=element_text(size=10,face="bold"),
   ...
   annotate(geom="text",x =( as.Date(int.start.one)+(int.end-int.start.one)/2),
             y=(y.lim2[2]/100)*99, vjust=1, hjust=0.5,
             label=int.descrip,fontface="italic",color="grey25",size=3.5)+
  geom_ribbon(aes(ymin=lower.CI,ymax=upper.CI, 
  ...
  scale_x_date(limits=as.Date(c("2020-11-01","2021-07-01")),date_breaks = "1 month",date_labels = "%b %Y",expand=c(0,0))+
  scale_y_continuous(expand=c(0,0),breaks=seq(0,y.lim2[2],2))+
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
                              title.position = "top",title="Global origin",
                              legend.spacing=0,ncol=1),
         alpha="none", color="none")+
  coord_cartesian(ylim=y.lim2)
p1.sing.line.ci
ggsave(paste(f.out,"/Rolling-singleton-importRate_line_confintALL.png",sep=""),height=3,width=3.5)
...
if(int.yn==T){
   ...
   ### NOW FOR INTERVENTION 1, b/c gamma is annoying like that:
   rate.pre<-sing.Par.Roll.summary %>% filter(par.state%in%focal.source) %>% filter(tmrca.dt.half==int.start.one)
   
   rate.2wk<-sing.Par.Roll.summary %>% filter(par.state%in%focal.source) %>% filter(tmrca.dt.half==(int.start.one+14))
   
    rate.4wk<-sing.Par.Roll.summary %>% filter(par.state%in%focal.source) %>% filter(tmrca.dt.half==(int.start.one+28))
   
   cat(paste0("Weekly singles importation rate pre-int 1 from ",focal.source, 
              " on ",int.start.one,": ", 
              mean.95ci.givenmsd(m=rate.pre$intros_meansum7d.mean,
                                    sd=rate.pre$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   
   cat(paste0("Weekly singles importation rate 2-weeks post-int 1 from ",focal.source, 
              " on ",int.start.one+14, ": ", 
              mean.95ci.givenmsd(m=rate.2wk$intros_meansum7d.mean,
                                    sd=rate.2wk$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   ...
   ## FOLD DECREASE from pre to 2wk, intervention 1
   fc2wk.focal<-c()
   fc4wk.focal<-c()
   ...
   for (k in 1:n.B){
     temp<-par.state.sing.boots.full[[k]] %>% 
      dplyr::filter(par.state%in%focal.source) %>%
      as.data.frame()
     temp.pre<-temp %>% filter(tmrca.dt.half==int.start.one)
     temp.2wk<-temp %>% filter(tmrca.dt.half==(int.start.one+14))
     temp.4wk<-temp %>% filter(tmrca.dt.half==(int.start.one+28))
   ...
   cat(paste0("Fold decrease singles rate pre-int1 to 2-weeks after int1 from ",focal.source, 
              ": ", mean.fc.2wk),    file=text.out,sep="\n",append=T)
   
   cat(paste0("Fold decrease singles rate pre-int1 to 4-weeks after int1 from ",focal.source, 
              ": ", mean.fc.4wk),    file=text.out,sep="\n",append=T)
...
if(int.yn==T){
  ...
  ### NOW FOR INTERVENTION 1, b/c GAMMA IS BLAHH
   #how many singletons introduced before intervention 1
  single.preban<-c()
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.start.one)))
    single.preban<-c(single.preban,x)}
  cat(paste0('Mean (95%CI) # of singles pre-intervention 1 = ',
       mean.95ci.X(single.preban,0) ), 
    file=text.out,sep="\n",append=T)
  
  #how many singletons introduced before intervention 1 from focal
  single.preban1<-c()
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.start.one) &
                      can.sing.boots[[k]]$par.state %in%focal.source))
    single.preban1<-c(single.preban1,x) }
   cat(paste0('Mean (95%CI) # of singles pre-intervention 1 from focal = ',
         mean.95ci.X(single.preban1,0)  ), 
    file=text.out,sep="\n",append=T) 
  ...
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.start.one) &
                      can.sing.boots[[k]]$par.state %in%focal.source))
    y<-length(which(can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.start.one)))
    prop<-x/y
    single.preban.prop<-c(single.preban.prop, prop) }
  cat(paste0('Proportion of singles pre-intervention 1 from focal = ',
         mean.95ci.X(single.preban.prop,2)  ), 
  file=text.out,sep="\n",append=T) 
  ...
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half>=as.Date(int.start.one) &
                    can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.end.one)  ))
    single.duringban<-c(single.duringban,x)}
   cat(paste0('Mean (95%CI) # of singles during intervention 1 = ',
        mean.95ci.X(single.duringban,0) ), 
    file=text.out,sep="\n",append=T) 
  ...
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half>=as.Date(int.start.one) &
                    can.sing.boots[[k]]$tmrca.dt.half<as.Date(int.end.one) &
                      can.sing.boots[[k]]$par.state %in%focal.source))
    single.duringban1<-c(single.duringban1,x) }
   cat(paste0('Mean (95%CI) # of singles during intervention 1 from focal = ',
         mean.95ci.X(single.duringban1,0)  ), 
    file=text.out,sep="\n",append=T) 
   ...
   #proportion of single during ban from focal
    ...
    prop<-x/y
    single.durban.prop<-c(single.durban.prop, prop) }
   cat(paste0('Proportion of singles during-intervention 1 from focal = ',
         mean.95ci.X(single.durban.prop,2)  ), 
    file=text.out,sep="\n",append=T) 
  ...
  single.afterban<-c()
  for (k in 1:n.B){
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half>=as.Date(int.end.one)  ))
    single.afterban<-c(single.afterban,x)}
    cat(paste0('Mean (95%CI) # of singles after intervention 1 = ',
           mean.95ci.X(single.afterban,0)  ), 
    file=text.out,sep="\n",append=T) 
  ...
  single.afterban.prop<-c()
    ...
    x<-length(which(can.sing.boots[[k]]$tmrca.dt.half>=as.Date(int.end.one) &
                      can.sing.boots[[k]]$par.state %in%focal.source))
    y<-length(which(can.sing.boots[[k]]$tmrca.dt.half>=as.Date(int.end.one) ))
    prop<-x/y
    single.afterban.prop<-c(single.afterban.prop, prop) }
   cat(paste0('Proportion of singles post-intervention 1 from focal = ',
         mean.95ci.X(single.afterban.prop,2)  ), 
    file=text.out,sep="\n",append=T) 
  ...
  for (k in 1:n.B){
    ...
    fc<-x/y #if pre=0.5 and during=0.1, 0.5/0.1 = 5-fold reduction 
    single.fc.prop<-c(single.fc.prop, fc) }
  cat(paste0('Fold reduction in proportion of singles pre vs during-intervention 1 from focal = ',
       mean.95ci.X(single.fc.prop,2)  ), 
  file=text.out,sep="\n",append=T) 
...
p.focal.var.cases
ggsave(paste0(f.out,"p.focal.var.cases.short-x.png"),height=3,width=4,unit="in")
```

## FOR FIRST RESTRICTION
...
# 
# #make a summary df of n in each category for text and pos
# sum.boots.binSize.pre.summ<-sum.boots.binSize.pre%>% 
#   dplyr::group_by(Number.Desc.Factor) %>%
#   dplyr::summarise(.groups="rowwise",total.subs=sum(n)) %>% 
#   as.data.frame()
# 
# levels(sum.boots.binSize.pre$Number.Desc.Factor)
# 
# #PLOT the grouped sizes
# P3.pre<-ggplot(data=sum.boots.binSize.pre)+
#   geom_bar(aes(x=Number.Desc.Factor,y=n, group=Lineage,fill=Lineage),
#                position="stack",stat="identity",alpha=0.9)+
#   pubTheme+
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
#         axis.text.x=element_text(angle=45,vjust=1,hjust=1,size=rel(1.2)),
...
# 
# P3.pre 
# ggsave(paste(f.out,"/Sublineagesizes.bylin.Frequencies.untilendrestric1.png",sep=""),width=4,height=4,units="in")
...
P3.smooth
ggsave(paste(f.out,"/Sublineagesizes.Smooth.Frequencies.untilend.restric1.png",sep=""),width=4,height=4,units="in")

```

## fit and predictions for first restriction
```{r}
# sum.Par.Roll.summary<-read.csv("2023-07-06_gamma_analysis/sum.par.roll.summary.csv")
sum.Par.Roll.summary$tmrca.dt<-as.Date(sum.Par.Roll.summary$tmrca.dt)
...
focal.var.cases.lim<-focal.var.cases[which(focal.var.cases$avgVOC_cases>5000),]
#don't use sparse data to fit
focal.cases.intros<-left_join(sum.Par.Roll.focal.pre,focal.var.cases.lim, by="date")
# head(focal.cases.intros)

#Make a linear model
#use this to predict the number of sublineages averted based on cases
lm.cases.intros1<-lm(data=focal.cases.intros, intros_meansum7d.mean ~ avgVOC_cases)
sum.lm.cases.intros1<-summary(lm.cases.intros1) #Adjusted R-squared:  0.9531 ,  p-value: < 2.2e-16
sum.lm.cases.intros1
adjustedr2<-round(sum.lm.cases.intros1$r.squared,digits = 3)
text.adjustedr2<-paste0("Multiple R-squared = ",adjustedr2)
#Relationship b/w cases in the UK And intros into Canada
p.focal.cases.intros.restriction1<-ggplot(focal.cases.intros,
                             aes(x=avgVOC_cases,
  ...
  pubTheme+
  annotate(geom="label",
           x=5000,y=max(focal.cases.intros$intros_meansum7d.mean,na.rm=T),
           label=text.adjustedr2,hjust=0)

p.focal.cases.intros.restriction1

ggsave(paste0(f.out,"p.focal.var.cases.intros.lm.restriction1.png"),height=3,width=3,unit="in")
...
new.vals<-predict.lm(lm.cases.intros1,newdata = focal.cases.df,interval = "prediction")
...
focal.cases.intros2.new<-predict.lm(lm.cases.intros1,
                                    newdata = focal.cases.intros2,
                                    interval = "prediction")
...
p.focal.obs.pred.restriction1<-ggplot(focal.obs.pred,
...
p.focal.obs.pred.restriction1
ggsave(paste0(f.out,"p.obs.pred.restriction1.png"),height=3,width=3,unit="in")

## Join the predicted values onto the data over time

#get dates back in here
focal.cases.df3.restriction1<-focal.cases.df2 %>% left_join(focal.var.cases[,c("date","avgVOC_cases")],by="avgVOC_cases")
focal.cases.df3.restriction1$type<-"Predicted"
...
y.lim<-c(0,max(focal.cases.df3.restriction1$intros_meansum7d.mean)+1)
p.focal.pred.sublin.restriction1<-focal.intros.all %>%
  ggplot()+
  pubThemeDate+
  annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim[1], ymax =y.lim[2],
  ...
  labs(x="Date of most recent common ancestor",y=paste0("Sublineages per week"))+
  scale_x_date(date_labels = "%b %Y",
                            limits=as.Date(c("2020-11-14","2021-06-01")),
                            expand=c(0,0))+
  scale_y_continuous(expand=c(0,0))+
  #observed
  ...
  #predicted
  geom_density(data=focal.cases.df3.restriction1,aes(x=date,y=intros_meansum7d.mean,
...
p.focal.pred.sublin.restriction1
ggsave(paste0(f.out,"p.focal.pred.sublin.restriction1.png"),height=3,width=3,unit="in")

#no legend version
p.focal.pred.sublin.noleg<-p.focal.pred.sublin.restriction1+
  theme(legend.position = "none")
p.focal.pred.sublin.noleg
ggsave(paste0(f.out,"p.focal.pred.sublin.no.leg.restriction1.png"),height=3,width=3,unit="in")
...
AUC.observed.restriction1<-sum(diff(observed.df$date[id])*
...
AUC.observed.restriction1 #observed # of sublineages

### Predicted:
predicted.df<-focal.cases.df3.restriction1 %>% filter(date<=int.end.one & date>=int.start.one) %>% dplyr::select (c("date","intros_meansum7d.mean"))
id<-order(predicted.df$date)
AUC.predicted.restriction1<-sum(diff(predicted.df$date[id])*
                  ((rollmean(predicted.df$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
AUC.predicted.restriction1 #observed # of sublineages

## differences
averted.sublin.restriction1<-AUC.predicted.restriction1 - AUC.observed.restriction1
averted.sublin.restriction1
```
...
ymax<-0.25
p.averted1.restriction1<-ggplot(cases.averted.all.df.pre) +
  ...
  annotate(geom = "text",x =mean+100,y=ymax*0.8,
           label=paste0("Mean cases averted (95% CI)\n= ",confint),
           hjust=-0.05,size=3)+
  pubTheme
  # theme(axis.title.x=element_text(margin = margin(t = -30))) #run for grob only
p.averted1.restriction1
ggsave(paste0(f.out,"cases.averted.allsourcesendrestric.restriction1.png"),height=2.7,width=3,unit="in")

#make a version in the variant color
var.col<-varPalette.ch[which(names(varPalette.ch)==Focal.var)]
p.averted1.restriction1.varcol<-ggplot(cases.averted.all.df.pre) +
  geom_density(aes(x=cases.averted.all,y=..count..),
  ...
  pubTheme
  # theme(axis.title.x=element_text(margin = margin(t = -30))) #run for grob only
p.averted1.restriction1.varcol
ggsave(paste0(f.out,"cases.averted.allsourcesendrestric.restriction1_varcol.png"),height=2.7,width=3,unit="in")
...
ymax<-0.5
p.avert2.restriction1<-ggplot(cases.averted.focal.df) +
  geom_density(aes(x=cases.averted.all,y=..count..),
               color="chartreuse3",fill="chartreuse3",alpha=0.5)+
  labs(x="Cases averted",y="Frequency")+
  ...
  annotate(geom = "text",x =mean+100,y=ymax*0.8,
           ...
           hjust=-0.05)+
  pubTheme
p.avert2.restriction1
ggsave(paste0(f.out,"cases.averted.focalsourcesendrestric.restriction1.png"),height=3,width=4,unit="in")
...
plot_grid(p.focal.var.cases, p.focal.obs.pred, 
          p.focal.pred.sublin.restriction1, p.averted1.restriction1, nrow=2,
          labels=LETTERS[1:4],
          align="hv")
...
P3.smooth
ggsave(paste(f.out,"/Sublineagesizes.Smooth.Frequencies.untilend.restric2.png",sep=""),width=4,height=4,units="in")

```

## fit and predictions for restriction 2
```{r}
#sublin rate right before the intervetion:
sum.Par.Roll.focal.pre<-sum.Par.Roll.summary %>%
  filter(Parent.Location%in%focal.source) %>%
  filter(tmrca.dt > int.end.one) %>% #after first intervention
  filter(tmrca.dt < int.start) #before second

## join the focal cases to the importation rate pre-intervention
sum.Par.Roll.focal.pre$date<-sum.Par.Roll.focal.pre$tmrca.dt

# class(focal.var.cases$tmrca.dt)
focal.var.cases.lim2<-focal.var.cases[which(focal.var.cases$avgVOC_cases<55000),]
focal.cases.intros<-left_join(sum.Par.Roll.focal.pre,
                              focal.var.cases.lim2, by="date")
# head(focal.cases.intros)

#Make a linear model
#use this to predict the number of sublineages averted based on cases
lm.cases.intros2<-lm(data=focal.cases.intros, intros_meansum7d.mean ~ avgVOC_cases)
sum.lm.cases.intros2<-summary(lm.cases.intros2) #Adjusted R-squared:  0.9531 ,  p-value: < 2.2e-16
sum.lm.cases.intros2
adjustedr2<-round(sum.lm.cases.intros2$r.squared,digits = 3)
text.adjustedr2<-paste0("Multiple R-squared = ",adjustedr2)
#Relationship b/w cases in the UK And intros into Canada
p.focal.cases.intros<-ggplot(focal.cases.intros,
  ...
  pubTheme+
  annotate(geom="label",
           x=20000,y=max(focal.cases.intros$intros_meansum7d.mean,na.rm=T),
           label=text.adjustedr2,hjust=0)
...
#use the older lm instead (better fit)
new.vals<-predict.lm(lm.cases.intros2,
                     newdata = focal.cases.df,
                     interval = "prediction")
#join new vals on with the new cases data
focal.cases.df2<-focal.cases.df %>% bind_cols(new.vals)
colnames(focal.cases.df2)[2:4]<-c("intros_meansum7d.mean","lower","upper")
...
## Join this onto the other dataset, and distinguish them as 'no restriction' (predicted) and 'restrictions' (observed)
### JOIN ON TO THE PREVIOUS DATASET OF OBSERVED AND PREDICTED
focal.cases.intros2<-focal.cases.intros[,c("avgVOC_cases","intros_meansum7d.mean","country")]
focal.cases.intros2$type<-"Observed"

## Also need to add confidence intervals to the observed data that was fitted
focal.cases.intros2.new<-predict.lm(lm.cases.intros,
                                    newdata = focal.cases.intros2,
                                    interval = "prediction")
...
#this df has 
focal.obs.pred.both<-bind_rows(focal.obs.pred, #first predictions and observed
                               focal.cases.df2)
...
## Plot and color by type
p.focal.obs.pred.both<-ggplot(focal.obs.pred.both,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=type)) +
  geom_ribbon(aes(ymin = lower,ymax = upper,fill=type),alpha=0.5)+
  geom_point(aes(color=type),alpha=0.8)+
  labs(x=paste0(focal.source.brf, ": Average daily ",Focal.var," cases"), y="Sublineages per week")+
...
p.focal.obs.pred
ggsave(paste0(f.out,"p.obs.pred.restriction2.png"),height=3,width=3,unit="in")

## Join the predicted values onto the data over time

#get dates back in here
focal.cases.df3.restriction2<-focal.cases.df2 %>%
  left_join(focal.var.cases[,c("date","avgVOC_cases")],
            by="avgVOC_cases")
focal.cases.df3.restriction2$type<-"Predicted"
# head(focal.cases.df3)
...
pred.pal2<-pred.pal
names(pred.pal2)<-c("Observed","Predicted")
y.lim<-c(0,max(focal.cases.df3$intros_meansum7d.mean)+1)
p.focal.pred.sublin.both<-focal.intros.all %>%
  ggplot()+
  pubThemeDate+
  annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim[1], ymax =y.lim[2],
  ...
  #observed
   geom_density(aes(x=date,y=intros_meansum7d.mean,color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  #predicted, restriction 1
  geom_density(data=focal.cases.df3.restriction1,aes(x=date,y=intros_meansum7d.mean,
                                        color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  #predicted, restriction 2
  geom_density(data=focal.cases.df3.restriction2,aes(x=date,y=intros_meansum7d.mean,
                                        color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  ...
  scale_color_manual(values=pred.pal2)+
  scale_fill_manual(values=pred.pal2)
p.focal.pred.sublin.both
ggsave(paste0(f.out,"p.focal.pred.sublin.bothrestrict.png"),height=3,width=3,unit="in")

#no legend version
p.focal.pred.sublin.noleg.both<-p.focal.pred.sublin.both+
  theme(legend.position = "none")
p.focal.pred.sublin.noleg.both
ggsave(paste0(f.out,"p.focal.pred.sublin.both.no.leg.png"),height=3,width=3,unit="in")
...
```{r}
#integrate stepwise daily 
observed.df2<-focal.intros.all %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean"))
id<-order(observed.df2$date)
AUC.observed.restriction2<-sum(diff(observed.df2$date[id])*
                  ((rollmean(observed.df2$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
#divided by seven to convert it to per day instead of per week
AUC.observed.restriction2 #observed # of sublineages

### Predicted:
predicted.df2<-focal.cases.df3.restriction2 %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean"))
id<-order(predicted.df2$date)
AUC.predicted.restriction2<-sum(diff(predicted.df2$date[id])*
                  ((rollmean(predicted.df2$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
AUC.predicted.restriction2 #observed # of sublineages

## differences
averted.sublin.restriction2<-AUC.predicted.restriction2 - AUC.observed.restriction2
averted.sublin.restriction2
averted.sublin.restriction1
...
## Calculate cases averted: sum of sublin sizes averted
```{r}
## fit a gamma model to the observed distribution of sublinaege sizes caluclate above 
# Moment matching estimation consists in equalizing theoretical and empirical moments. Estimated values of the distribution parameters are computed by a closed-form formula
ans.pre <- fitdist(as.numeric(sum.boots.binSize.pre$N.Desc.glob), 
               distr = "gamma", method = "mme", #start=c(1,1000),
               discrete=T)
...
ans.focalpre <- fitdist(as.numeric(sum.boots.binSize.focalpre$N.Desc.glob), 
               ...
               discrete=T)
#run function to draw
cases.averted.all.df.pre.restriction2<-draw.averted.cases(N=averted.sublin.restriction2,
                                             gamma.df=ans.pre)

#plot it
confint<-mean.95ci.X(cases.averted.all.df.pre.restriction2$cases.averted.all)
mean<-mean(cases.averted.all.df.pre.restriction2$cases.averted.all)  
ymax<-0.65
p.averted1.restriction2<-ggplot(cases.averted.all.df.pre.restriction2) +
  geom_density(aes(x=cases.averted.all,y=..count..),
               color="chartreuse3",fill="chartreuse3",alpha=0.5)+
  labs(x="Cases averted",y="Frequency")+
  geom_vline(xintercept = mean, linetype=2)+
  annotate(geom = "text",x =mean+100,y=ymax*0.9,
           label=paste0("Mean cases averted (95% CI)\n= ",confint),
           hjust=-0.05,size=3)+
  pubTheme
  # theme(axis.title.x=element_text(margin = margin(t = -30))) #run for grob only
p.averted1.restriction2
ggsave(paste0(f.out,"cases.averted.allsourcesendrestric.restriction2.png"),height=2.7,width=3,unit="in")

#make a version in the variant color
var.col<-varPalette.ch[which(names(varPalette.ch)==Focal.var)]
p.averted1.restriction2.varcol<-ggplot(cases.averted.all.df.pre.restriction2) +
  geom_density(aes(x=cases.averted.all,y=..count..),
               color=var.col,fill=var.col,alpha=0.5)+   
  # geom_density(aes(x=cases.averted.all,y=..count..),
  #              color="chartreuse3",fill="chartreuse3",alpha=0.5)+
  ...
  pubTheme
  # theme(axis.title.x=element_text(margin = margin(t = -30))) #run for grob only
p.averted1.restriction2.varcol
ggsave(paste0(f.out,"cases.averted.allsourcesendrestric.restriction2_varcol.png"),height=2.7,width=3,unit="in")

##repeat for focal only gamma distribution of sublin sizees
cases.averted.focal.df.restriction2<-draw.averted.cases(N=averted.sublin.restriction2,
                                           gamma.df = ans.focalpre)
#plot it
confint<-mean.95ci.X(cases.averted.focal.df.restriction2$cases.averted.all)
mean<-mean(cases.averted.focal.df.restriction2$cases.averted.all)  
ymax<-1.2
p.avert2.restriction2<-ggplot(cases.averted.focal.df.restriction2) +
  geom_density(aes(x=cases.averted.all,y=..count..),
               color="chartreuse3",fill="chartreuse3",alpha=0.5)+
  labs(x="Cases averted",y="Frequency")+
  geom_vline(xintercept = mean, 
  ...
  annotate(geom = "text",x =mean+100,y=ymax,
           ...
           hjust=-0.05)+
  pubTheme
p.avert2.restriction2
ggsave(paste0(f.out,"cases.averted.focalsourcesendrestric.restriction2.png"),height=3,width=4,unit="in")

```

## Grob together the key plots from this analysis
```{r}
plot_grid(p.focal.var.cases, p.focal.obs.pred.both, 
          p.focal.pred.sublin.both, p.averted1.restriction2, nrow=2,labels=LETTERS[1:4],
          align="hv")
ggsave(paste0(f.out,"composite_casesAverted_full_restrictboth.png"),height=7,width=7,units = "in")
...
cat(paste0("Sublineages observed during restriction 1: ",
           round(AUC.observed.restriction1,digits = 1)),
    paste0("Sublineages predicted (from mean sublin rate) during restriction 1: ",
           round(AUC.predicted.restriction1,digits = 1)),
    paste0("Sublineages averted (from mean sublin rate) during restriction 1: ", 
           round(averted.sublin.restriction1,digits = 1)),
    paste0("Mean cases (sublin desc) averted during restriction 1, 
           all source distrib, mean: ",
           mean.95ci.X(cases.averted.all.df.pre$cases.averted.all,1)),
    paste0("Mean cases (sublin desc) averted during restriction 1, 
           focal distrib, mean: ",
           mean.95ci.X(cases.averted.focal.df$cases.averted.all,1)),
    
    paste0("Sublineages observed during restriction 2: ",
           round(AUC.observed.restriction2,digits = 1)),
    paste0("Sublineages predicted (from mean sublin rate) during restriction 2: ",
           round(AUC.predicted.restriction2,digits = 1)),
    paste0("Sublineages averted (from mean sublin rate) during restriction 2: ", 
           round(averted.sublin.restriction2,digits = 1)),
    paste0("Mean cases (sublin desc) averted during restriction 2, 
           all source distrib, mean: ",
           mean.95ci.X(cases.averted.all.df.pre.restriction2$cases.averted.all,1)),
    paste0("Mean cases (sublin desc) averted during restriction 2, 
           focal distrib, mean: ",
           mean.95ci.X(cases.averted.focal.df.restriction2$cases.averted.all,1)),
    
    paste0("Sublineages averted (from mean sublin rate) during BOTH restrictions: ", 
           round(averted.sublin.restriction2+averted.sublin.restriction1,digits = 1)),
    paste0("Mean cases (sublin desc) averted during BOTH restrictions, 
           all source distrib, mean: ",
           mean.95ci.X((cases.averted.all.df.pre$cases.averted.all+
                        cases.averted.all.df.pre.restriction2$cases.averted.all),1)),
    paste0("Mean cases (sublin desc) averted during BOTH restrictions, 
           focal distrib, mean: ",
           mean.95ci.X((cases.averted.focal.df$cases.averted.all+
                        cases.averted.focal.df.restriction2$cases.averted.all),1)),
    file=text.out, append=T,sep="\n")
```
...
```{r}
## THERE were no identified singletons before the first intervention
#singleton rate right before the intervention:
sing.Par.Roll.focal.pre<-sing.Par.Roll.summary %>% 
  filter(par.state%in%focal.source) %>%
  # filter(tmrca.dt.half > int.end.one)
...
#Make a linear model
#remove NAs
# focal.cases.intros.sing<-na.omit(focal.cases.intros.sing)
...
## DURING RESTRICTION 2 and 1 (at the same time)
focal.cases.new<-focal.var.cases %>% filter((date<=int.end & date>=int.start) | 
                                            (date<=int.end.one & date>=int.start.one )) 
...
#get dates back in here
focal.cases.df.sing3.restriction2<-focal.cases.df.sing2 %>% left_join(focal.var.cases[,c("date","avgVOC_cases")],by="avgVOC_cases")
focal.cases.df.sing3.restriction2$type<-"Predicted"
# head(focal.cases.df.sing3.restriction2)
...
# colnames(focal.cases.df.sing3.restriction2)

## MAke preditions for the first restriciton
...
pred.pal2<-pred.pal
names(pred.pal2)<-c("Observed","Predicted")
y.lim<-c(0,max(focal.cases.df.sing3.restriction2$intros_meansum7d.mean)+1)
p.sing.focal.pred.singleton<-focal.intros.sing.all %>%
  ...
  pubThemeDate+
   annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim[1], ymax =y.lim[2],
             color="grey85",size=0,  fill="grey80")+
   annotate(geom="rect",xmin = as.Date(int.start2), xmax = as.Date(int.end2),
             ymin = y.lim[1], ymax =y.lim[2],
             color="grey85",size=0,  fill="grey80")+
    annotate(geom="text",x =( as.Date(int.start2)+(int.end-int.start2)/2),
             y=(y.lim[2]/100)*99, vjust=1, hjust=0.5,
             label=int.descrip,fontface="italic",color="grey25",size=3)+
  ...
  #observed
   geom_density(aes(x=date,y=intros_meansum7d.mean,color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  #predicted
  geom_density(data=focal.cases.df.sing3.restriction2[(focal.cases.df.sing3.restriction2$date<=int.end.one),],
               aes(x=date,y=intros_meansum7d.mean,
                                        color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  #separate two restricts
  geom_density(data=focal.cases.df.sing3.restriction2[(focal.cases.df.sing3.restriction2$date>=int.start),],
               aes(x=date,y=intros_meansum7d.mean,
                                        color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  guides(color=guide_legend(title="Scenario"),
...
p.sing.focal.pred.singleton+theme(legend.position = "none")
ggsave(paste0(f.out,"p.sing.focal.pred.singleton.noleg.png"),height=3.5,width=3.5,unit="in")
...
id<-order(observed.df$date)
AUC.observed.sing.restriction2<-sum(diff(observed.df$date[id])*
                  ((rollmean(observed.df$intros_meansum7d.mean[id],2))/7) ,na.rm=T) %>% as.numeric()
#divided by seven to convert it to per day instead of per week
AUC.observed.sing.restriction2 #observed # of singletons

#integrate stepwise daily , RESTRICT 1
observed.df<-focal.intros.sing.all %>% filter(date<=int.end.one & date>=int.start.one) %>% dplyr::select (c("date","intros_meansum7d.mean"))
id<-order(observed.df$date)
AUC.observed.sing.restriction1<-sum(diff(observed.df$date[id])*
                  ((rollmean(observed.df$intros_meansum7d.mean[id],2))/7),na.rm=T ) %>% as.numeric()
#divided by seven to convert it to per day instead of per week
AUC.observed.sing.restriction1 #observed # of singletons


### Predicted: restriction2
predicted.df<-focal.cases.df.sing3.restriction2 %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean"))
id<-order(predicted.df$date)
AUC.predicted.sing.restriction2<-sum(diff(predicted.df$date[id])*
                  ((rollmean(predicted.df$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
AUC.predicted.sing.restriction2 #pred # of singletons

### Predicted, restriction1
predicted.df<-focal.cases.df.sing3.restriction2%>% 
  filter(date<=int.end.one & date>=int.start.one) %>% 
  dplyr::select (c("date","intros_meansum7d.mean"))

id<-order(predicted.df$date)
AUC.predicted.sing.restriction1<-sum(diff(predicted.df$date[id])*
                  ((rollmean(predicted.df$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
AUC.predicted.sing.restriction1 #pred# of singletons
AUC.predicted.sing.restriction2

## differences
averted.singleton.restriction2<-AUC.predicted.sing.restriction2 - AUC.observed.sing.restriction2

averted.singleton.restriction2 #2.68674

averted.singleton.restriction1<-AUC.predicted.sing.restriction1 - AUC.observed.sing.restriction1

averted.singleton.restriction1 #5.694655

totsingz<-round(sum(averted.singleton.restriction2,averted.singleton.restriction1))
```
...
## Add to text export
```{r}
cat(paste0("Singletons observed during restriction 1: ",
           round(AUC.observed.sing.restriction1,digits=1)),
    paste0("Singletons predicted (from mean single rate) during restriction 1: ",
           round(AUC.predicted.sing.restriction1,digits=1)),
    paste0("Singletons averted (from mean single rate) during restriction 1: ", 
           round(averted.singleton.restriction1,digits=1)),
    
    paste0("Singletons observed during restriction 2: ",
           round(AUC.observed.sing.restriction2,digits=1)),
    paste0("Singletons predicted (from mean single rate) during restriction 2: ",
           round(AUC.predicted.sing.restriction2,digits=1)),
    paste0("Singletons averted (from mean single rate) during restriction 2: ", 
           round(averted.singleton.restriction2,digits=1)),
    
    paste0("TOTAL Singletons averted (from mean single rate) during BOTH restrictions: ", 
           round(sum(averted.singleton.restriction1,averted.singleton.restriction2),digits=1)),
    
## TOTAL CASES AVERTED= sublineages + singeltons
    paste0("TOTAL mean cases averted, sublins+singles, all source distrib: ",
           round(mean(cases.averted.all.df.pre$cases.averted.all+
                        cases.averted.all.df.pre.restriction2$cases.averted.all)+totsingz, digits=1)),
    paste0("TOTAL mean cases averted, sublins+singles, focal distrib: ",
           round(mean(cases.averted.focal.df$cases.averted.all+
                        cases.averted.focal.df.restriction2$cases.averted.all)+totsingz,
           digits=1)),
   file=text.out, append=T,sep="\n")
...
tots.averted<-round(mean(cases.averted.all.df.pre$cases.averted.all+
                         cases.averted.all.df.pre.restriction2$cases.averted.all)+totsingz, digits=1)
percent.focal.averted<-round(tots.averted/can.var.total.cases*100,digits=1)

## Plot cases with overlay for flight ban
y.lim2<-c(0,(ceiling(max(can.var.cases$avgVOC_cases,na.rm=T))+1700))
...
p.can.var.cases<-ggplot(can.var.cases)+
   ...
   annotate(geom="rect",xmin = as.Date(int.start.one), xmax = as.Date(int.end.one),
    ...
    annotate(geom="label",x =( as.Date(int.start.one)+(int.end-int.start.one)/2),
             y=(y.lim2[2]/10)*9, vjust=1, hjust=0.5,
