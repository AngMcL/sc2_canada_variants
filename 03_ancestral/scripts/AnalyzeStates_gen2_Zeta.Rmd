...
ft_trees_pre<-"../02_trees/202208_analysis/ft_omicronzeta_root_res"

focal.source<-c("Brazil")
focal.source.brf<-c("Brazil") 
...
#sublineage from Africa
sublin.Africa<-c()
for (k in 1:n.B){
  x<-sum.boots[[k]] %>%
    filter(Parent.Location=="Africa") %>%
    nrow()
  sublin.Africa<-c(sublin.Africa,x)}
cat(paste0('Mean (95% CI) # of Africa-origin variant sublineages = ',
      mean.95ci.X(sublin.Africa,0) ), 
    file=text.out,sep="\n",append=T)
cat(paste0('Percent of Africa-origin variant sublineages = ',
      round((mean(sublin.Africa)/mean(sublin.tot)*100 ),digits=2) ), 
    file=text.out,sep="\n",append=T)
...
p.ridge<-ggplot(temp.df, aes(x=Date, y=Sublineage,fill=Lineage),alpha=0.7) + 
  ...
  scale_x_date(date_labels = "%b %Y",expand=c(0,0),limits=as.Date(c("2020-08-01","2021-09-01")))+
...
P1.singz<- ggplot(sing.Par.Prov.summary.long,
  ...
  GlobFillScale
P1.singz
ggsave(paste(f.out,"alluvial.singletons.Parent.Node.png",sep=""),height=5,width=5,units="in")
```
