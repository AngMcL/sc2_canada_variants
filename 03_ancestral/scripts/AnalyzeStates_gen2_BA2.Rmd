...
ft_trees_pre<-paste0("../02_trees/BA_analysis/ft_",focal.var,"_root_res")
...
meta.can.cases.in<-"../01_subSample/Results_PreSubsamp_omi_2023/meta.can.var.prov.daily.full3.csv"
...
scaleDateFlexLess<-scale_x_date(date_breaks = "1 month", 
                                date_labels = "%b %Y",
                                expand=c(0,0),                                
...
if(int.yn==T){
  ...
  ggsave(paste(f.out,"SepWaves.Singles.Alluvial.percent.Parent.Node.png",sep=""),height=5,width=9,units="in")
}
```
...
p1.sing.line.ci<-sing.Par.Roll.summary %>%
  ...
  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0),breaks=seq(0,y.lim2[2],10))+
...
totalintros %>%
  ggplot()+
  geom_bar(aes(x=tmrca.dt,y=total.intros),stat="identity",width=1,color="#f0aea7")+
  pubThemeDate+
  scaleDateFlex+
  labs(x=NULL, y="Total daily introductions")+
  theme(legend.position = "top")
...
colnames(focal.cases.df2)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df2$country<-focal.source
focal.cases.df2$type<-"Predicted (no restrictions)"      
...
focal.cases.intros.sing2$type<-"Observed (restrictions)"
...
colnames(focal.cases.df.sing2)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df.sing2$country<-focal.source
focal.cases.df.sing2$type<-"Predicted (no restrictions)"      
...
focal.cases.df.sing3$type<-"Predicted (no restrictions)"
