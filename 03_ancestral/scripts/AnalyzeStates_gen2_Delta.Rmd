...
fig.end.date<-as.Date("2021-12-01")
...
# f.out<-"2023-07-06_delta_analysis/"
...
sum.Par.Roll.summary$focal[which(sum.Par.Roll.summary$Parent.Location%in%focal.source)]<-as.factor(1)
# table(sum.Par.Roll.summary$focal)
...
```{r}
# sum.Par.Roll.summary<-read.csv("2023-07-06_delta_analysis/sum.par.roll.summary.csv")
sum.Par.Roll.summary$focal<-as.factor(sum.Par.Roll.summary$focal)
...
##PLOT AS LINE WITH CONFIDENCE INTERVALS
alphavals<-c(0.3,0.9)
names(alphavals)<-as.factor(c("0","1"))
...
p1.line.ci<-sum.Par.Roll.summary %>%
 ...
 scale_x_date(date_labels = "%b %Y", date_breaks="1 month",
              limits=as.Date(c("2020-12-14","2022-02-01")))+        scale_y_continuous(expand=c(0,0),breaks=seq(0,y.max.singles,y.jumps))+
 guides(fill = guide_legend(keywidth = 0.4,keyheight=0.4,
                              title.position = "top",title="Global origin",
                              legend.spacing=0,ncol=1,
                              # override.aes = list(alpha = c(0.3,0.9,0.3,0.3,0.3,
                                                            # 0.3,0.3,0.3,0.3,0.3,
                                                            # 0.3,0.3,0.3))
                            ),
        color="none",
  ...
  coord_cartesian(ylim=y.lim2)
p1.line.ci
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confintALL_delta.png",sep=""),height=3,width=3.5)
...
p1.line.ci.noleg<-sum.Par.Roll.summary %>%
  ...
  annotate(geom="rect",xmin = as.Date(int.start),
           ...
           color="grey85",size=0,  fill="grey80")+
  annotate(geom="text",x = as.Date(int.start)+int.duration/2,
           y=y.lim2[2]-0.5, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
  geom_ribbon(aes(ymin=lower.CI,ymax=upper.CI,
                  fill=Parent.Location, alpha=focal))+
  geom_line(stat="identity",lwd=1,aes(alpha=focal,color=Parent.Location))+
  GlobRegColScale+
  GlobRegFillScale+
  # scale_alpha_discrete(alphavals)+
  scale_alpha_manual(values=alphavals)+
  labs(x="Date of most recent common ancestor",y=paste0("Sublineages per week"), fill="Origin Location")+
  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0),breaks=seq(0,y.max.singles,y.jumps))+
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
         ...
         alpha="none", color="none")+
  coord_cartesian(ylim=y.lim2)+
  annotate(geom="text",x=as.Date("2021-04-06"),y=20, label="India",
           color=globalPalette.ch[which(names(globalPalette.ch)=="India")],size=4)+
  annotate(geom="text",x=as.Date("2021-07-23"),y=9.2, label="USA",
           color=globalPalette.ch[which(names(globalPalette.ch)=="USA")],alpha=0.8,size=2.5)+
   annotate(geom="text",x=as.Date("2021-07-29"),y=3.5,label="Europe",
...
## version with label
plot_grid(p1.line.ci.noleg, labels="D) Delta",label_x=-0.1)
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confint_noleg_D.png",sep=""),height=3,width=3.5)
...
```{r}
# sing.Par.Roll.summary<-read.csv("2023-07-06_delta_analysis/sing.par.roll.summary.csv")
...
p1.sing.line.ci<-sing.Par.Roll.summary %>%
  ...
  # scaleDateFlex+
 scale_x_date(date_labels = "%b %Y", date_breaks="1 month",
              limits=as.Date(c("2020-12-14","2022-02-01")))+      
 ...
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
  ...
  coord_cartesian(ylim=y.lim2)
p1.sing.line.ci
ggsave(paste(f.out,"/Rolling-singleton-importRate_line_confintALL_delta.png",sep=""),height=3,width=3.5)

## export a version with a legend a
legend.in<-"2023-07-05_BA11_analysis/countryleg.png"
p.legend<-cowplot::ggdraw() + cowplot::draw_image(legend.in, scale = 1)
plot_grid(NULL,p.legend, NULL, p1.sing.line.ci, labels=NULL,
          align="hv", axis="top",
          rel_heights=c(-0.2,0.5,-0.2,1),nrow=4)

ggsave(paste(f.out,"/Rolling-singleton-importRate_line_confint_leg.png",sep=""),height=3,width=3.5)
#Line no CI
...
sing.Roll.total<-sing.Par.Roll.summary %>% dplyr::group_by(tmrca.dt.half) %>%
  dplyr::summarize(.groups="rowwise",
                   total_meansum7d.mean=(sum(intros_meansum7d.mean,na.rm=T)),
                   total_meansum7d.sd=(sum(intros_meansum7d.sd,na.rm=T))) %>%
  as.data.frame()
...
### NEWLY ADDED FOR DELTA
## summarize fold change rolling singleton rates
```{r}
#overall sum of new singles/week in Canada max

max.overall<-head(na.omit(sing.Roll.total[rev(order(sing.Roll.total$total_meansum7d.mean)),]),n=1)
dt.temp<-max.overall[1,1]
cat(paste0("Max weekly single importation rate overall on ", dt.temp,": ",
           mean.95ci.givenmsd(m=max.overall[2],sd=max.overall[3],n=n.B,2)),
    file=text.out,sep="\n",append=T)
...
cat(paste0("Max weekly single importation rate from ",focal.source, " on ",dt.focal,
           ": ", mean.95ci.givenmsd(m=max.mean.focal,sd=max.sd.focal, n.B, 2) ),
    file=text.out,sep="\n",append=T)
...
max.sd.usa<-sing.Par.Roll.summary$intros_meansum7d.sd[which(sing.Par.Roll.summary$intros_meansum7d.mean ==max.mean.usa )]
dt.usa<-sing.Par.Roll.summary$tmrca.dt.half[which(sing.Par.Roll.summary$intros_meansum7d.mean==max.mean.usa) ] 
cat(paste0("Max weekly single importation rate from ","USA", " on ",dt.usa,
           ": ", mean.95ci.givenmsd(m=max.mean.usa,sd=max.sd.usa, n.B, 2) ),
    file=text.out,sep="\n",append=T)

## IF there was a var-specifc intervention, calculate single import on date implemented vs two weeks after from focal source
if(int.yn==T){
   rate.pre<-sing.Par.Roll.summary %>% filter(par.state%in%focal.source) %>% filter(tmrca.dt.half==int.start)
   ...
   cat(paste0("Weekly single importation rate pre-int from ",focal.source, 
              " on ",int.start,": ", 
              mean.95ci.givenmsd(m=rate.pre$intros_meansum7d.mean,
                                    sd=rate.pre$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   
   cat(paste0("Weekly single importation rate 2-weeks post-int from ",focal.source, 
              " on ",int.start+14, ": ", 
              mean.95ci.givenmsd(m=rate.2wk$intros_meansum7d.mean,
                                    sd=rate.2wk$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   
   cat(paste0("Weekly single importation rate 4-weeks post-int from ",focal.source, 
              " on ",int.start+28, ": ", 
              mean.95ci.givenmsd(m=rate.4wk$intros_meansum7d.mean,
                                    sd=rate.4wk$intros_meansum7d.sd, n.B, 2) ),
    file=text.out,sep="\n",append=T)
   ...
   cat(paste0("Fold decrease single rate pre-int to 2-weeks after int from ",focal.source, 
              ": ", mean.fc.2wk),    file=text.out,sep="\n",append=T)
   
   cat(paste0("Fold decrease single rate pre-int to 4-weeks after int from ",focal.source, 
              ": ", mean.fc.4wk),    file=text.out,sep="\n",append=T)
...
NOTE THAT THIS IS IN A DIFFERENT PROJECT NOW FOR PRIAMRY ANALYSIS
...
for (k in 1:n.B){
  sum.boots[[k]]<-read.csv(sum.out[k])
}
...
P3.pre<-ggplot(data=sum.boots.binSize.pre)+
  ...
  guides(fill=guide_legend(ncol= 1,title="Lineage",title.position="top",reverse = T, keywidth = 1.2,keyheight = 1.2))+
  geom_text(data=sum.boots.binSize.summ,aes(y=total.subs,x=Number.Desc.Factor,label=paste(total.subs,sep=""),hjust=0.5,vjust=-0.3),size=2.7)+  
  scale_y_continuous(limits=c(0,225),expand=c(0.02,0))
  # annotate("rect",xmin=2.5,xmax=16.4,ymin=166,ymax=202, fill="white")
...
### THIS FUNCTIONAL FORM IS NO BUENO
