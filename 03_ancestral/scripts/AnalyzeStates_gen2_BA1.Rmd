...
focal.var<-"BA1"
Focal.var<-"BA.1"
ft_trees<-paste0("../02_trees/BA_analysis/",focal.var, "_lsd_out")
ft_in<-paste0("2022_BA_ace_out/",focal.var,"_ace_out")
ft_trees_pre<-"../02_trees/BA_analysis/ft_BA1_root_res"
...
fig.end.date<-as.Date("2022-02-01")
...
file.exists(variant.df.in);file.exists(variant.df.in.glob)

# folder for exports
today<-Sys.Date()
f.out<-paste0(today,"_",focal.var,"_analysis/") 
# f.out<-"2023-07-05_ba1_analysis/"
...
#version with spaces for long names
globalPalette.ch.sp<-globalPalette.ch
names(globalPalette.ch.sp)<-str_replace_all(names(globalPalette.ch.sp), 
                                            c("British Columbia"="British\nColumbia",
                                              "United Kingdom"="UK",
                                              "Newfoundland and Labrador"="Newfoundland\nand Labrador"))

GlobColScale.sp<-scale_colour_manual(name = "Location",values = globalPalette.ch.sp,na.value="grey60")
GlobFillScale.sp<-scale_fill_manual(name = "Location",values = globalPalette.ch.sp,na.value="grey60")
...
#varaint specific colors
if(str_detect("BA",Focal.var)){
  focal.col<-rownames(varPalette)[which(tolower(varPalette$vars.ch)=="omicron")]
}
...
scaleDateFlexMore<-scale_x_date(date_breaks = "1 month", date_labels = "%b %Y",expand=c(0,0),
                                limits=c(fig.start.date,data.date))
...
#   myroot<-decimal_date(as.Date(first(sort(td$date))))
  ...
  ### THIS IS ACTUALLY 4/5 NOW, b/c was biasing too early
...
# VarColScale<-scale_colour_manual(name = "Location",values =linPalette.ch,na.value="grey60")
# VarFillScale<-scale_fill_manual(name = "Location",values = linPalette.ch,na.value="grey60")
...
if(int.yn==T){
  ...
  sublin.fc.prop<-na.omit(sublin.fc.prop) %>% as.numeric()
...
P1.count<- ggplot(sum.Par.Prov.summary.long,
  ...
  geom_flow(alpha = 0.6,width=0.45) +
  geom_stratum(alpha = 0.9,width=0.45) +
  geom_text(stat = "stratum", size = 3.4,min.y=15,fontface="bold") +
  pubTheme+
  theme(legend.position = "none", axis.line = element_blank(), text =element_text(size=14),
        axis.ticks.x = element_blank(),axis.text.x = element_text(hjust=c(0.5,0.5)))+
  labs(x=NULL,y=paste0("# sublineages"))+
...
if(int.yn==T){
  ...
  sum.Par.Prov.summary$Node.Location<-str_replace_all(sum.Par.Prov.summary$Node.Location,"Newfoundland and Labrador","Newfoundland")
  ...
  names(globalPalette.ch.mod)<-str_replace_all(names(globalPalette.ch.mod),
                                               c("British Columbia"="British\nColumbia",
                                                 "Newfoundland and Labrador","Newfoundland"
                                                 focal.source=focal.source))
...
if(mean(un.lins)>1){
  ...
  alluv.lin.n<-ggplot(sum.Par.Prov.L.summary.long,
    ...
    scale_color_manual(name = "geo",values = globalPalette.ch,na.value="grey60")+
    geom_text(stat = "stratum", size = 3.3,min.y=20,label.padding=unit(0.1, "lines")) +
    geom_text(stat="flow",aes(label=Lineage,color=Lineage),size=1.5,min.y=5,color="white",
              nudge_x=-0.206,hjust=1)+
    pubTheme+
    theme(legend.position = "none",
          axis.ticks.x = element_blank(),axis.text.x = element_text(hjust=c(0.5,0.5)),
...
P3<-ggplot(data=sum.boots.binSize)+
  ...
  scale_y_continuous(limits=c(0,220),expand=c(0.02,0.01))
...
p1<-sum.Par.Roll.summary %>%
  ...
  annotate(geom="text",x = as.Date(int.start),y=y.lim[2], vjust=1, hjust=0,
           label=int.descrip,fontface="italic",color="grey25",size=2.5)+
  ...
  scaleDateFlex+
  scale_y_continuous(expand=c(0.1,0),limits=y.lim,breaks=seq(0,max(y.lim),y.jumps1))+
...
y.lim2<-c(0,(y.max.singles+10))

sum.Par.Roll.summary2<-sum.Par.Roll.summary
notfocal<-which(!sum.Par.Roll.summary2$Parent.Location %in% c(focal.source,"USA"))
sum.Par.Roll.summary2$upper.CI[notfocal]<-NA
sum.Par.Roll.summary2$lower.CI[notfocal]<-NA

p1.line.ci.noleg<-sum.Par.Roll.summary2 %>%
  ggplot(aes(x=tmrca.dt,y=intros_meansum7d.mean,group=Parent.Location))+
  theme(text=element_text(size=10,face="bold"),
  ...
  annotate(geom="rect",xmin = as.Date(int.start), 
           ...
           color="grey85",size=0,  fill="grey80")+
  annotate(geom="text",x = as.Date(int.start)+int.duration/2,
           y=y.lim2[2]-2, vjust=1, hjust=0.5,
 ...
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
         ...
         alpha="none", color="none")+
  coord_cartesian(ylim=y.lim2)+
  annotate(geom="text",x=as.Date("2021-12-24"),y=4, label="South Africa",
           color=globalPalette.ch[which(names(globalPalette.ch)=="South Africa")],size=4)+
  annotate(geom="text",x=as.Date("2021-12-14"),y=20, label="UK",
           color=globalPalette.ch[which(names(globalPalette.ch)=="United Kingdom")],alpha=0.8,size=2.5)+
  annotate(geom="text",x=as.Date("2021-11-25"),y=25,label="USA",
           color=globalPalette.ch[which(names(globalPalette.ch)=="USA")],alpha=0.8,size=2.5)+
  annotate(geom="text",x=as.Date("2021-11-27"),y=11,label="Europe",
...
## version with label
plot_grid(p1.line.ci.noleg, labels="E) BA.1",label_x=-0.1)
ggsave(paste(f.out,"/Rolling-sublin-importRate_line_confint_noleg_E.png",sep=""),height=3,width=3.5)
...
P5<-ggplot(sum.boots[[k]])+
  ...
  scale_color_manual(values=c("deepskyblue3"))+
  geom_ribbon(data=newdata2,aes(x=tmrca.dt, ymin=(LL),ymax=(UL)),fill="deepskyblue3",alpha = 0.25,) +
  geom_line(data=newdata2,aes(x=tmrca.dt, y=longevityTMRCA), color="deepskyblue3",size = 0.5,alpha=0.5,) +
  pubThemeDate+
  theme(legend.position="none")+
  labs(x="Date of most recent common ancestor",y="Sublineage longevity (days)")+
  scaleDateFlex+
  scale_y_continuous(expand=c(0.01,0), limits=c(-50,190))+
    coord_cartesian(clip="on",ylim=c(0,190))
...
ggsave(paste(f.out,"/SublineageCartoonExpansion.png",sep=""),height=6,width=8.5,units="in")
...
if(int.yn==T){
  ...
  ggsave(paste(f.out,"SepWaves.Alluvial.Singles.percent.Parent.Node.png",sep=""),height=5,width=11,units="in")
}
```
...
p1.sing<-sing.Par.Roll.summary %>%
 ...
 annotate(geom="text",x = (as.Date(int.start)+int.duration/2),
  ...
  scale_y_continuous(expand=c(0,0),limits=y.lim,breaks=seq(0,y.upper,50))+
...
p1.sing.line.ci<-sing.Par.Roll.summary %>%
 ...
 guides(fill = guide_legend(keywidth = 0.6,keyheight=0.6,
         ...
         alpha="none", color="none")+
  coord_cartesian(ylim=y.lim2)+
    scale_y_continuous(expand=c(0,0),breaks=seq(0,y.lim2[2],50))
p1.sing.line.ci
...
write.csv(sum.boots.binSize.pre, paste0(f.out,"sum.boots.binSize.pre.",focal.var,".csv"))
...
P3.pre<-ggplot(data=sum.boots.binSize.pre)+
  ...
  guides(fill=guide_legend(ncol= 1,title="Lineage",title.position="top",reverse = T, keywidth = 1.2,keyheight = 1.2))+
  geom_text(data=sum.boots.binSize.summ,aes(y=total.subs,x=Number.Desc.Factor,label=paste(total.subs,sep=""),hjust=0.5,vjust=-0.3),size=2.7)+  
  scale_y_continuous(limits=c(0,525),expand=c(0.02,0))
  # annotate("rect",xmin=2.5,xmax=16.4,ymin=166,ymax=202, fill="white")
...
```{r}
# sum.Par.Roll.summary<-read.csv("2023-02-10_delta_analysis/sum.par.roll.summary.csv")
...
p.focal.var.cases<-ggplot(focal.var.cases)+
  ...
  GlobFillScale+
  theme(legend.position = "none")+
  labs(x=NULL, y=paste0(focal.source.brf, ": Average\ndaily ",Focal.var," cases"))
...
# #need to merge the two lcoations for cases
# focal.var.cases.both<-focal.var.cases %>% dplyr::group_by(date) %>%
#   dplyr::summarise(avgVOC_cases=sum(avgVOC_cases,na.rm=T))

# class(focal.var.cases$tmrca.dt)
focal.var.cases$Parent.Location<-focal.var.cases$country
focal.cases.intros<-left_join(sum.Par.Roll.focal.pre,focal.var.cases, by=c("date","Parent.Location"))
# head(focal.cases.intros)

#Make a linear model for each focal source
#use this to predict the number of sublineages averted based on cases
focal.cases.intros.red<-focal.cases.intros %>% filter(avgVOC_cases>10) 
focal.cases.intros.red.afr<-focal.cases.intros %>% filter(Parent.Location=="Africa") 
focal.cases.intros.red.safr<-focal.cases.intros %>% filter(Parent.Location=="South Africa") 

lm.cases.intros.afr<-lm(data=focal.cases.intros.red.afr, intros_meansum7d.mean ~ avgVOC_cases)
sum.lm.cases.intros.afr<-summary(lm.cases.intros.afr) #
sum.lm.cases.intros.afr
adjustedr2.afr<-round(sum.lm.cases.intros.afr$r.squared,digits = 3)
text.adjustedr2.afr<-paste0("Multiple R-squared = ",adjustedr2.afr)


lm.cases.intros.safr<-lm(data=focal.cases.intros.red.safr, intros_meansum7d.mean ~ avgVOC_cases)
sum.lm.cases.intros.safr<-summary(lm.cases.intros.safr) 
sum.lm.cases.intros.safr
adjustedr2.safr<-round(sum.lm.cases.intros.safr$r.squared,digits = 3)
text.adjustedr2.safr<-paste0("Multiple R-squared = ",adjustedr2.safr)


#Relationship b/w cases and intros into Canada
p.focal.cases.intros<-focal.cases.intros %>% 
  filter(country=="South Africa") %>% #added b/c no Africa imports
  ggplot(aes(x=avgVOC_cases,
  ...
  geom_smooth(data=focal.cases.intros.red.safr, method ="lm",se=T,level=0.95)+
  pubTheme+
  #r sq for africa
  # annotate(geom="label",
  #          x=1,y=max(focal.cases.intros$intros_meansum7d.mean,na.rm=T),
  #          label=text.adjustedr2.afr,hjust=0)+
  #and annotation for south africa
  annotate(geom="label",
           x=50,y=max(focal.cases.intros$intros_meansum7d.mean,na.rm=T)/3*2.5,
           label=text.adjustedr2.safr,hjust=0)

p.focal.cases.intros

ggsave(paste0(f.out,"p.focal.var.cases.intros.lm.png"),height=3,width=4,unit="in")

#now predict number of sublins per week up until int.end
# FIrst for africa

focal.cases.new.afr<-focal.var.cases %>% filter(date<=int.end & date>=int.start) %>% 
  filter(Parent.Location=="Africa")
focal.cases.df.afr<-focal.cases.new.afr$avgVOC_cases %>% as.data.frame()
colnames(focal.cases.df.afr)<-"avgVOC_cases"

new.vals.afr<-predict.lm(lm.cases.intros.afr,newdata = focal.cases.df.afr,interval = "prediction")
#join new vals on with the new cases data
focal.cases.df2.afr<-focal.cases.df.afr %>% bind_cols(new.vals.afr)
colnames(focal.cases.df2.afr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df2.afr$country<-focal.source[2]

#plot this to see how many sublins there might have been during the ban
p.obs.fit.afr<-ggplot(focal.cases.df2.afr,
...
focal.cases.new.safr<-focal.var.cases %>% filter(date<=int.end & date>=int.start) %>% 
  filter(Parent.Location=="South Africa")
focal.cases.df.safr<-focal.cases.new.safr$avgVOC_cases %>% as.data.frame()
colnames(focal.cases.df.safr)<-"avgVOC_cases"

new.vals.safr<-predict.lm(lm.cases.intros.safr,newdata = focal.cases.df.safr,interval = "prediction")
#join new vals on with the new cases data
focal.cases.df2.safr<-focal.cases.df.safr %>% bind_cols(new.vals.safr)
colnames(focal.cases.df2.safr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df2.safr$country<-focal.source[1]

#plot this to see how many sublins there might have been during the ban
p.obs.fit.safr<-ggplot(focal.cases.df2.safr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=country)) +
    geom_ribbon(aes(ymin = lower,ymax = upper,fill=country),alpha=0.3)+
  geom_point(aes(color=country))+
  GlobColScale+
...
focal.cases.intros2$type<-"Observed (restrictions)"
#africa
# focal.cases.intros2.afr<-focal.cases.intros2 %>% filter(country=="Africa")
#south africa
focal.cases.intros2.safr<-focal.cases.intros2 %>% filter(country=="South Africa")


## Also need to add confidence intervals to the observed data that was fitted
# focal.cases.intros2.new.afr<-predict.lm(lm.cases.intros.afr,newdata = focal.cases.intros2.afr,interval = "prediction")
# #only want the upper and lower here
# focal.cases.intros2.new.afr<-focal.cases.intros2.new.afr[,c(2,3)]
# colnames(focal.cases.intros2.new.afr)<-c("lower","upper")
# focal.cases.intros3.afr<-focal.cases.intros2.afr %>% bind_cols(focal.cases.intros2.new.afr)

#join new vals on with the new cases data
# focal.cases.df2.afr<-focal.cases.df.afr %>% bind_cols(new.vals.afr)
# colnames(focal.cases.df2.afr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
# focal.cases.df2.afr$country<-focal.source[2]
# focal.cases.df2.afr$type<-"Predicted (no restrictions)"      
# focal.obs.pred.afr<-bind_rows(focal.cases.intros3.afr,focal.cases.df2.afr)

## Also need to add confidence intervals to the observed data : South safr
focal.cases.intros2.new.safr<-predict.lm(lm.cases.intros.safr,newdata = focal.cases.intros2.safr,interval = "prediction")
#only want the upper and lower here
focal.cases.intros2.new.safr<-focal.cases.intros2.new.safr[,c(2,3)]
colnames(focal.cases.intros2.new.safr)<-c("lower","upper")
focal.cases.intros3.safr<-focal.cases.intros2.safr %>% bind_cols(focal.cases.intros2.new.safr)

#join new vals on with the new cases data
focal.cases.df2.safr<-focal.cases.df.safr %>% bind_cols(new.vals.safr)
colnames(focal.cases.df2.safr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df2.safr$country<-focal.source[1]
focal.cases.df2.safr$type<-"Predicted (no restrictions)"      
focal.obs.pred.safr<-bind_rows(focal.cases.intros3.safr,focal.cases.df2.safr)


## colors for this
pred.pal<-c("deepskyblue4","chartreuse3")
names(pred.pal)<-c("Observed (restrictions)","Predicted (no restrictions)")

## Plot and color by type
# p.focal.obs.pred.afr<-ggplot(focal.obs.pred.afr,
...
#   labs(x=paste0(focal.source[2], ": Average daily ",Focal.var," cases"), y="Sublineages per week")+
...
#     annotate(geom="label",
#            x=max(focal.obs.pred.afr$avgVOC_cases,na.rm=T),
#            y=mean(focal.obs.pred.afr$intros_meansum7d.mean,na.rm=T)+5,
#            label=text.adjustedr2.afr,hjust=1,size=3)+
#   scale_y_continuous(limits=c(0,30))
# 
# p.focal.obs.pred.afr
# ggsave(paste0(f.out,"p.obs.pred.afr.png"),height=3,width=3,unit="in")

## SOUTH AFRICA
## Plot and color by type
p.focal.obs.pred.safr<-ggplot(focal.obs.pred.safr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=type)) +
  geom_ribbon(aes(ymin = lower,ymax = upper,fill=type),alpha=0.5)+
  geom_point(aes(color=type),alpha=0.8)+
  labs(x=paste0(focal.source[1], ": Average daily ",Focal.var," cases"), y="Sublineages per week")+
  # geom_smooth(method ="lm",alpha=0.2,color=type)+
  pubTheme+
  theme(legend.position = c(0.3,0.8),
        #axis.title.x=element_text(margin = margin(t = -30))
  ...
  guides(color=guide_legend(title="Scenario"),
         fill=guide_legend(title="Scenario"))+ 
    annotate(geom="label",
           x=max(focal.obs.pred.safr$avgVOC_cases,na.rm=T)*0.8,
           y=mean(focal.obs.pred.safr$intros_meansum7d.mean,na.rm=T),
           label=text.adjustedr2.safr,hjust=1,size=3)+
  coord_cartesian(y=c(0,5))

p.focal.obs.pred.safr
ggsave(paste0(f.out,"p.obs.pred.safr.png"),height=3,width=3,unit="in")

## Join the predicted values onto the data over time

#get dates back in here
# focal.cases.df2.afr$country

# focal.cases.df3.afr<-focal.cases.df2.afr %>% left_join(focal.var.cases[,c("date","avgVOC_cases","country")],by=c("avgVOC_cases","country"))

focal.cases.df3.safr<-focal.cases.df2.safr %>% left_join(focal.var.cases[,c("date","avgVOC_cases","country")],by=c("avgVOC_cases","country"))

#join them together
# focal.cases.df3<-bind_rows(focal.cases.df3.afr, focal.cases.df3.safr)
focal.cases.df3<- focal.cases.df3.safr

focal.cases.df3$type<-"Predicted (no restrictions)"
# head(focal.cases.df3)

focal.intros.all<-sum.Par.Roll.summary %>% 
  filter(Parent.Location%in%focal.source[1])
focal.intros.all$type<-"Observed (restrictions)"
...
pred.pal2<-pred.pal
names(pred.pal2)<-c("Observed (restrictions)","Predicted (no restrictions)")
...
p.focal.pred.sublin<-focal.intros.all %>%
  ...
  scaleDateFlex
p.focal.pred.sublin
ggsave(paste0(f.out,"p.focal.pred.sublin.png"),height=3.5,width=3.5,unit="in")
...
observed.df<-focal.intros.all %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean","Parent.Location"))
#sum for two countries
observed.df2<-observed.df%>%dplyr::group_by(date) %>% dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm=T))
id<-order(observed.df2$date)

AUC.observed<-sum(diff(observed.df2$date[id])*
...
### Predicted:
predicted.df<-focal.cases.df3 %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean","country"))

#sum for two countries
predicted.df2<-predicted.df%>%dplyr::group_by(date) %>% dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm=T))
id<-order(predicted.df2$date)

AUC.predicted<-sum(diff(predicted.df2$date[id])*
...
var.col<-morevarPal[which(names(morevarPal)==Focal.var)]
...
```{r}
plot_grid(p.focal.var.cases, p.focal.obs.pred.safr, 
...
## Make changes to consider africa and south africa separately, then merged together
```{r}
...
# class(focal.var.cases$tmrca.dt)
sing.Par.Roll.focal.pre$country<-sing.Par.Roll.focal.pre$par.state
focal.cases.intros.sing<-left_join(sing.Par.Roll.focal.pre, focal.var.cases, by=c("date","country"))
# head(focal.cases.intros.sing)

#Make a linear model for separate focal
focal.cases.intros.sing.afr<-focal.cases.intros.sing %>% filter(country=="Africa")
focal.cases.intros.sing.safr<-focal.cases.intros.sing %>% filter(country=="South Africa")

#use this to predict the number of singletons averted based on cases
lm.cases.intros.sing.afr<-lm(data=focal.cases.intros.sing.afr, intros_meansum7d.mean ~ avgVOC_cases)
sing.lm.cases.intros.sing.afr<-summary(lm.cases.intros.sing.afr) #Adjusted R-squared:  0.9531 ,  p-value: < 2.2e-16
sing.lm.cases.intros.sing.afr
adjustedr2.afr<-round(sing.lm.cases.intros.sing.afr$r.squared,digits = 3)
text.adjustedr2.afr<-paste0("Multiple R-squared = ",adjustedr2.afr)
#Relationship b/w cases in the UK And intros into Canada
p.sing.focal.cases.intros.sing.afr<-ggplot(focal.cases.intros.sing.afr,
                             aes(x=avgVOC_cases,
                                 y=intros_meansum7d.mean,color=country))+
  geom_point()+
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=paste0(focal.source[2], ": Average daily ",Focal.var," cases"), 
...
p.sing.focal.cases.intros.sing.afr
ggsave(paste0(f.out,"p.sing.focal.var.cases.intros.sing.lm.afr.png"),height=3,width=3,unit="in")

#for S africa
focal.cases.intros.sing.safr.red<-focal.cases.intros.sing.safr #%>% filter(avgVOC_cases <1)
# focal.cases.intros.sing.safr.red<-focal.cases.intros.sing.safr.red[which(!is.na(focal.cases.intros.sing.safr.red$intros_meansum7d.mean)),]
# focal.cases.intros.sing.safr.red<-focal.cases.intros.sing.safr.red[which(focal.cases.intros.sing.safr.red$intros_meansum7d.mean==0),]
lm.cases.intros.sing.safr<-lm(data=focal.cases.intros.sing.safr.red, intros_meansum7d.mean ~ avgVOC_cases)
sing.lm.cases.intros.sing.safr<-summary(lm.cases.intros.sing.safr)
sing.lm.cases.intros.sing.safr
adjustedr2.safr<-round(sing.lm.cases.intros.sing.safr$r.squared,digits = 3)
text.adjustedr2.safr<-paste0("Multiple R-squared = ",adjustedr2.safr)
#Relationship b/w cases in the UK And intros into Canada
p.sing.focal.cases.intros.sing.safr<-ggplot(focal.cases.intros.sing.safr,
                             aes(x=avgVOC_cases,
                                 y=intros_meansum7d.mean,color=country))+
  geom_point()+
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=paste0(focal.source[1], ": Average daily ",Focal.var," cases"), 
       y="Singletons per week")+
  geom_smooth(data=focal.cases.intros.sing.safr,method ="lm",se=T,level=0.95)+
  pubTheme+
  annotate(geom="label",
           x=1,y=max(focal.cases.intros.sing$intros_meansum7d.mean,na.rm=T),
           label=text.adjustedr2.safr,hjust=0)

p.sing.focal.cases.intros.sing.safr
ggsave(paste0(f.out,"p.sing.focal.var.cases.intros.sing.lm.safr.png"),height=3,width=3,unit="in")
...
#for africa
focal.cases.new.afr<-focal.cases.new %>% filter(country=="Africa")
focal.cases.df.sing.afr<-focal.cases.new.afr$avgVOC_cases %>% as.data.frame()
colnames(focal.cases.df.sing.afr)<-"avgVOC_cases"
new.vals.afr<-predict.lm(lm.cases.intros.sing.afr,newdata = focal.cases.df.sing.afr,interval = "prediction")

#join new vals on with the new cases data
focal.cases.df.sing2.afr<-focal.cases.df.sing.afr %>% bind_cols(new.vals.afr)
colnames(focal.cases.df.sing2.afr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df.sing2.afr$country<-focal.source[2]

#plot this to see how many singletons there might have been during the ban
p.sing.obs.fit.afr<-ggplot(focal.cases.df.sing2.afr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=country)) +
    geom_ribbon(aes(ymin = lower,ymax = upper,fill=country),alpha=0.3)+
  geom_point(aes(color=country))+
  GlobColScale+
...
#for safrica
focal.cases.new.safr<-focal.cases.new %>% filter(country=="South Africa")
focal.cases.df.sing.safr<-focal.cases.new.safr$avgVOC_cases %>% as.data.frame()
colnames(focal.cases.df.sing.safr)<-"avgVOC_cases"
new.vals.safr<-predict.lm(lm.cases.intros.sing.safr,newdata = focal.cases.df.sing.safr,interval = "prediction")

#join new vals on with the new cases data
focal.cases.df.sing2.safr<-focal.cases.df.sing.safr %>% bind_cols(new.vals.safr)
colnames(focal.cases.df.sing2.safr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df.sing2.safr$country<-focal.source[1]

#plot this to see how many singletons there might have been during the ban
p.sing.obs.fit.safr<-ggplot(focal.cases.df.sing2.safr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=country)) +
    geom_ribbon(aes(ymin = lower,ymax = upper,fill=country),alpha=0.3)+
  geom_point(aes(color=country))+
  GlobColScale+
...
focal.cases.intros.sing2.afr<-focal.cases.intros.sing.afr[,c("avgVOC_cases","intros_meansum7d.mean","country")]
focal.cases.intros.sing2.safr<-focal.cases.intros.sing.safr[,c("avgVOC_cases","intros_meansum7d.mean","country")]
focal.cases.intros.sing2.afr$type<-"Observed (restrictions)"
focal.cases.intros.sing2.safr$type<-"Observed (restrictions)"

## Also need to add confidence intervals to the observed data that was fitted
#AFRICA
focal.cases.intros.sing2.new.afr<-predict.lm(lm.cases.intros.sing.afr,newdata = focal.cases.intros.sing2.afr,interval = "prediction")
#only want the upper and lower here
focal.cases.intros.sing2.new.afr<-focal.cases.intros.sing2.new.afr[,c(2,3)]
colnames(focal.cases.intros.sing2.new.afr)<-c("lower","upper")
focal.cases.intros.sing3.afr<-focal.cases.intros.sing2.afr %>% bind_cols(focal.cases.intros.sing2.new.afr)

#join new vals on with the new cases data
focal.cases.df.sing2.afr<-focal.cases.df.sing.afr %>% bind_cols(new.vals.afr)
colnames(focal.cases.df.sing2.afr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df.sing2.afr$country<-focal.source[2]
focal.cases.df.sing2.afr$type<-"Predicted (no restrictions)"      
focal.obs.pred.afr<-bind_rows(focal.cases.intros.sing3.afr,focal.cases.df.sing2.afr)
#SOUTH AFRICA
focal.cases.intros.sing2.new.safr<-predict.lm(lm.cases.intros.sing.safr,newdata = focal.cases.intros.sing2.safr,interval = "prediction")
#only want the upper and lower here
focal.cases.intros.sing2.new.safr<-focal.cases.intros.sing2.new.safr[,c(2,3)]
colnames(focal.cases.intros.sing2.new.safr)<-c("lower","upper")
focal.cases.intros.sing3.safr<-focal.cases.intros.sing2.safr %>% bind_cols(focal.cases.intros.sing2.new.safr)

#join new vals on with the new cases data
focal.cases.df.sing2.safr<-focal.cases.df.sing.safr %>% bind_cols(new.vals.safr)
colnames(focal.cases.df.sing2.safr)[2:4]<-c("intros_meansum7d.mean","lower","upper")
focal.cases.df.sing2.safr$country<-focal.source[1]
focal.cases.df.sing2.safr$type<-"Predicted (no restrictions)"      
focal.obs.pred.safr<-bind_rows(focal.cases.intros.sing3.safr,focal.cases.df.sing2.safr)

#JOIN countries
focal.obs.pred<-bind_rows(focal.obs.pred.safr,focal.obs.pred.afr)
## colors for this
pred.pal<-c("deepskyblue4","chartreuse3")
names(pred.pal)<-c("Observed (restrictions)","Predicted (no restrictions)")

## Plot and color by type
p.sing.focal.obs.pred.afr<-ggplot(focal.obs.pred.afr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=type)) +
  geom_ribbon(aes(ymin = lower,ymax = upper,fill=type),alpha=0.5)+
  geom_point(aes(color=type),alpha=0.8)+
  labs(x=paste0(focal.source[2], ": Average daily ",Focal.var," cases"), y="Singletons per week")+
  # geom_smooth(method ="lm",alpha=0.2,color=type)+
  pubTheme+
  theme(legend.position = c(0.3,0.8),
        # axis.title.x=element_text(margin = margin(t = -30))
...
p.sing.focal.obs.pred.afr
ggsave(paste0(f.out,"p.sing.obs.pred.afr.png"),height=3,width=3,unit="in")



## Plot and color by type
p.sing.focal.obs.pred.safr<-ggplot(focal.obs.pred.safr,
       aes(x=avgVOC_cases,y=intros_meansum7d.mean,group=type)) +
  geom_ribbon(aes(ymin = lower,ymax = upper,fill=type),alpha=0.5)+
  geom_point(aes(color=type),alpha=0.8)+
  labs(x=paste0(focal.source[1], ": Average daily ",Focal.var," cases"), y="Singletons per week")+
  # geom_smooth(method ="lm",alpha=0.2,color=type)+
  pubTheme+
  theme(legend.position = c(0.3,0.8),
        # axis.title.x=element_text(margin = margin(t = -30))
...
p.sing.focal.obs.pred.safr
ggsave(paste0(f.out,"p.sing.obs.pred.safr.png"),height=3,width=3,unit="in")

## Join the predicted values onto the data over time

#get dates back in here
focal.cases.df.sing3.afr<-focal.cases.df.sing2.afr %>% left_join(focal.var.cases[,c("date","avgVOC_cases","country")],by=c("avgVOC_cases","country"))
focal.cases.df.sing3.safr<-focal.cases.df.sing2.safr %>% left_join(focal.var.cases[,c("date","avgVOC_cases","country")],by=c("avgVOC_cases","country"))

focal.cases.df.sing3.afr$type<-"Predicted (no restrictions)"
focal.cases.df.sing3.safr$type<-"Predicted (no restrictions)"

# head(focal.cases.df.sing3)
focal.cases.df.sing3<-bind_rows(focal.cases.df.sing3.afr,focal.cases.df.sing3.safr)

focal.intros.sing.all<-sing.Par.Roll.summary %>% 
  filter(par.state%in%focal.source)
focal.intros.sing.all$type<-"Observed (restrictions)"
...
focal.intros.sing.all.safr<-focal.intros.sing.all[which(focal.intros.sing.all$par.state==focal.source[1]),]
p.sing.focal.pred.singleton<-focal.intros.sing.all.safr %>%
  ggplot()+
  pubThemeDate+
  annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim[1], ymax =y.lim[2], color="grey85",size=0,  fill="grey80")+
  ...
  scale_x_date(breaks=xdates, date_labels = "%b %Y",
                            limits=as.Date(c("2021-01-01",data.date)),
                            expand=c(0,0))+
  scale_y_continuous(expand=c(0,0))+
  #observed
   geom_density(aes(x=date,y=intros_meansum7d.mean,color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
  #predicted
  geom_density(data=focal.cases.df.sing3.safr,aes(x=date,y=intros_meansum7d.mean,
                                        color=type,fill=type),
               stat="identity",size=1.5,alpha=0.7)+
...
```{r}
#integrate stepwise daily 
observed.df<-focal.intros.sing.all %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean","par.state"))

observed.df2<-observed.df %>% dplyr::group_by(date) %>% dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm=T))

id<-order(observed.df2$date)
AUC.observed.sing<-sum(diff(observed.df2$date[id])*
                  ((rollmean(observed.df2$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
#divided by seven to convert it to per day instead of per week
AUC.observed.sing #observed # of singletons

### Predicted:
predicted.df<-focal.cases.df.sing3 %>% filter(date<=int.end & date>=int.start) %>% dplyr::select (c("date","intros_meansum7d.mean","country"))

predicted.df2<-predicted.df %>% dplyr::group_by(date) %>% dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm=T))
id<-order(predicted.df2$date)
AUC.predicted.sing<-sum(diff(predicted.df2$date[id])*
                  ((rollmean(predicted.df2$intros_meansum7d.mean[id],2))/7) ) %>% as.numeric()
...
## TOTAL CASES AVERTED= sublineages + singeltons
    paste0("TOTAL mean cases averted, sublins+singles, all source distrib: ",
           round(mean(cases.averted.all.df.pre$cases.averted.all)+averted.singleton, digits=1)),
    paste0("TOTAL mean cases averted, sublins+singles, focal distrib: ",
           round(mean(cases.averted.focal.df$cases.averted.all)+averted.singleton, digits=1)),
   file=text.out, append=T,sep="\n")
...
y.lim2<-c(0,(ceiling(max(can.var.cases$avgVOC_cases,na.rm=T))+8800))
myvar<-morevarPal[which(names(morevarPal)==Focal.var)]
