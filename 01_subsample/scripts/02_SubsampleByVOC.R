...
align.out.folder<-"/Volumes/PhD_2/2022_SC2_Can/20220322_gisaid/01_subsampled/bootsamples_fasta/" 
meta.out.folder<-"bootsamples_meta/"
fig.out.folder<-"Results_Subsamp/"
...
t.other<-make.sample.t.VOCs.can("Other")
t.alpha<-make.sample.t.VOCs.can("Alpha")
t.beta<-make.sample.t.VOCs.can("Beta")
t.delta<-make.sample.t.VOCs.can("Delta")
t.epsilon<-make.sample.t.VOCs.can("Epsilon")
t.eta<-make.sample.t.VOCs.can("Eta")
t.gamma<-make.sample.t.VOCs.can("Gamma")
t.kappa<-make.sample.t.VOCs.can("Kappa")
t.iota<-make.sample.t.VOCs.can("Iota")
t.mu<-make.sample.t.VOCs.can("Mu")
t.omicron<-make.sample.t.VOCs.can("Omicron")
t.zeta<-make.sample.t.VOCs.can("Zeta")
...
t.long$paramlong<-str_replace_all(t.long$param, c("prop.avail"="Seq. available",
                                                            "prop.cases"="Cases",
                                                            "prop.take"="Seq. sampled"))


p.can.dis<-ggplot(t.long)+
  geom_density(aes(x=month, y=value,color=paramlong,group=paramlong),stat="identity",
               position=position_jitter(width = 0.2))+
  facet_wrap(~VOC,scales="free_y",nrow=5)+
...
t.glob.other<-make.sample.t.VOCs.glob(n.glob.other, "Other")
t.glob.alpha<-make.sample.t.VOCs.glob(n.glob.alpha, "Alpha")
t.glob.beta<-make.sample.t.VOCs.glob(n.glob.beta, "Beta")
t.glob.delta<-make.sample.t.VOCs.glob(n.glob.delta, "Delta")
t.glob.epsilon<-make.sample.t.VOCs.glob(n.glob.epsilon, "Epsilon")
t.glob.eta<-make.sample.t.VOCs.glob(n.glob.eta, "Eta")
t.glob.gamma<-make.sample.t.VOCs.glob(n.glob.gamma, "Gamma")
t.glob.iota<-make.sample.t.VOCs.glob(n.glob.iota, "Iota")
t.glob.kappa<-make.sample.t.VOCs.glob(n.glob.kappa, "Kappa")
t.glob.mu<-make.sample.t.VOCs.glob(n.glob.mu, "Mu")
t.glob.omicron<-make.sample.t.VOCs.glob(n.glob.omicron, "Omicron")
t.glob.zeta<-make.sample.t.VOCs.glob(n.glob.zeta, "Zeta")
...
t.glob<-t.glob[,-1]
t.glob<-t.glob %>% filter (VOC != "Omicron")
t.glob<-bind_rows(t.glob, t.globomi)
#' 
## ----------------------------------------------------------------------------------------------------------
#plot these out 
...
t.glob.long$paramlong<-str_replace_all(t.glob.long$param, c("prop.avail"="Seq. available",
                                                            "prop.cases"="Cases",
                                                            "prop.take"="Seq. sampled"))
...
## PLOT A GROB
library(cowplot)
p.can.dis.noleg<-p.can.dis+theme(legend.position = "none")
p.can.dis.low.leg<-p.can.dis+theme(legend.position = c(0.83,0.03))+
  guides(color=guide_legend(title.position="left",
                            title="Parameter",
                            
                            ))
p.glob.dis.noleg<-p.glob.dis+theme(legend.position = "none")

plot_grid(p.can.dis.low.leg, p.glob.dis.noleg, labels=c("A", "B"),
          rel_widths=c(1,1),nrow=1)
ggsave(paste0(fig.out.folder,"CanadaGlobal_distribution_202307.png"),width=14,height=5,units="in")
#' 
#' 
