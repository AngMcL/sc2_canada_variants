...
f.in2<-"Results_PreSubsamp_omi_2023/"
...
#modify var.ord
var.ord<-var.ord[-length(var.ord)] #remove omicron
var.ord.og<-var.ord # version without omicron to read in
omi.ord<-c("BA.1","BA.1.1","BA.2")
var.ord<-c(var.ord.og,omi.ord)

#single bootstrap, subsampled clean data for each VOC
#omi and other
metas.in<-c(paste0("bootsamples_meta_202208/",tolower(var.ord.og),"_1.csv"),
            paste0("bootsamples_meta_omi/",omi.ord,"_1.csv"))
...
prov.in<-paste0(f.in2,"/prov.csv")
#canada cases if needed?
can.in<-paste0(f.in2,"/can.csv")

#summarized global cases
global.cases.in<-paste0(f.in2,"/globalcases.csv")

#folder out
f.out<-"Results_PostSubsamp_202208_2023"
...
VarColScale<-scale_colour_manual(name = "Variant",values = morevarPal[match(var.ord,names(morevarPal))],na.value=NULL)
VarFillScale<-scale_fill_manual(name = "Variant",values = morevarPal[match(var.ord,names(morevarPal))],na.value=NULL)
```

## Read cases by province, VOC; daily and monthly
```{r}
...
## READ IN the separate BA ones, which also have all the other variants
meta.can.var.daily.full3.in<-paste0(f.in2,"/meta.can.var.daily.full3.csv") #estimated variant-specific cases for all of Can
meta.glob.var.daily.full3.in<-paste0(f.in2,"/meta.glob.var.daily.full3.csv") #estimated variant-specific cases for all of global

meta.can.var.prov.daily.full3.in<-paste0(f.in2,"/meta.can.var.prov.daily.full3.csv") #estimated variant-specific cases for each prov
meta.glob.var.country.daily.full3.in<-paste0(f.in2,"/meta.glob.var.country.daily.full3.csv")#estimated variant-specific cases for each country
...
meta.df$date<-as.Date(meta.df$date)
...
# CANADA STARTS HERE -skip for global
...
alldays<-seq(ymd(gdata::first(sort(meta.can.daily$date))),
...
#need to add rows for missing dates
alldays<-seq(ymd(gdata::first(sort(meta.can.var.daily$date))),
...
#need to add rows for missing dates
alldays<-seq(ymd(gdata::first(sort(meta.can.var.prov.daily$date))),
...
#remove unnecc obselete cols
meta.can.var.prov.daily.full3<-meta.can.var.prov.daily.full3[,-which(colnames(meta.can.var.prov.daily.full3) %in% c("avgtotal_cases","X","total","avgtotal","avgtotal_mod","total.day","prop"))]
# colnames(meta.can.var.prov.daily.full3)

### FOR SOME REASON THERE ARE DUPILCATES IN HERE AGGHRHH
# meta.can.var.prov.daily.full3[which(meta.can.var.prov.daily.full3$date=="2021-01-01" &
#       meta.can.var.prov.daily.full3$var.WHO=="Alpha"  &
#         meta.can.var.prov.daily.full3$division=="Ontario"),]

#find and DESTROYYY THEMMM
#make a united column 
meta.can.var.prov.daily.full3<-meta.can.var.prov.daily.full3 %>% unite(col="temp",1:3,remove = F)
head(meta.can.var.prov.daily.full3)
#then id dups of it
dupz<-which(duplicated(meta.can.var.prov.daily.full3$temp))
length(dupz)
#remove em
if(length(dupz)>0){
  meta.can.var.prov.daily.full3<-meta.can.var.prov.daily.full3[-dupz,]
}
#remove united column
meta.can.var.prov.daily.full3<-meta.can.var.prov.daily.full3[,-1]

#ADD the sampled equivalent
#use the summary df for total SEQUENCES per VAR in each division in each day
colnames(meta.can.var.prov.daily.full)
#check
# try<-meta.can.var.prov.daily.full.total %>% group_by(division,var.WHO) %>% summarize(.groups = "rowwise",totSeq=sum(total.day))

#replace NA and inf with 0
meta.can.var.prov.daily.full$var.WHO<-factor(meta.can.var.prov.daily.full$var.WHO,levels=var.ord)
meta.can.var.prov.daily.full$division<-factor(meta.can.var.prov.daily.full$division,levels=ord.prov)


meta.can.var.prov.daily.full3<-left_join(meta.can.var.prov.daily.full3, #cases
                                           meta.can.var.prov.daily.full, #seqs
                                         by=c("date","var.WHO","division"))

colnames(meta.can.var.prov.daily.full3)
##check
# try<-meta.can.var.prov.daily.full3%>% group_by(division,var.WHO) %>% summarize(.groups = "rowwise",totSeq=sum(total.daily.seqs,na.rm = T))
# head(try,n=20)
# ref<-meta.can %>% group_by(division,var.WHO) %>% count()
# ref[ref$division=="Ontario",] #Ontario  Alpha    8326 # Ontario  Other    7434
...
```{r}
#Add a month column
alldays<-seq(ymd(gdata::first(sort(prov$date))),
...
##check
# try<-meta.can.var.prov.daily.full3 %>% group_by(division, var.WHO) %>%
#   summarize(totseq=sum(total.daily.seqs,na.rm=T))
# head(try)
...
#check
# try<-meta.can.var.prov.daily.full.month %>% 
#   group_by(division, var.WHO) %>%
#   dplyr::summarise(.groups="rowwise",
#                    totalseqs=sum( totalVOCseqs))
# head(try,n=20)
...
meta.can.var.prov.daily.full.month %>%
  ...
  facet_wrap(~var.WHO,scales="free",ncol=5)+
    guides(color=guide_legend(title="Province",title.position = "left", 
                           keywidth = 0.8,keyheight = 0.8,nrow=2))
plot.seq.vs.case.per.month.div.voc
ggsave(paste(f.out,"/monthly.seq_per_case.byProv.byVOC.png",sep=""),width=8.5, height=5, units="in")  
...
summary.df.2<-meta.can.var.prov.daily.full.month.temp %>% group_by(var.WHO) %>%
  summarize(xpos=min(totalVOCcases,na.rm=T)+2, ypos=max(totalVOCseqs,na.rm=T))

meta.can.var.prov.daily.full.month.temp <- meta.can.var.prov.daily.full.month.temp%>%
  left_join(summary.df.2,  by="var.WHO")
...
meta.can.var.prov.daily.full.month.temp %>%
  ...
  ProvColScale2+
  pubTheme+
  theme(legend.position="none",legend.margin=margin(1,1,1,1,"pt"),
        legend.text=element_text(size=rel(0.7)),
        legend.title=element_text(size=rel(0.8)),
  ...
  guides(color=guide_legend(title="Province",title.position = "left", 
                           keywidth = 0.8,keyheight = 0.8,nrow=2))
plot.seq.vs.case.per.month.div.voc.cor
ggsave(paste(f.out,"/monthly.seq_per_case.byProv.byVOC.cor.png",sep=""),width=8.5, height=5, units="in")  

#version with B)
plot_grid(plot.seq.vs.case.per.month.div.voc.cor,labels="B")
ggsave(paste(f.out,"/monthly.seq_per_case.byProv.byVOC.cor.B.png",sep=""),width=8.5, height=5, units="in")  
...
alldays<-seq(ymd(gdata::first(sort(meta.glob.daily$date))),
...
#need to add rows for missing dates
alldays<-seq(ymd(gdata::first(sort(meta.glob.var.daily$date))),
...
#need to add rows for missing dates
alldays<-seq(ymd(gdata::first(sort(meta.glob.var.country.daily$date))),
...
plot.daily.ave.seqs.voc.country2<-meta.glob.var.country.daily.full%>%
  ...
  scaleDateBusy+
  facet_wrap(~var.WHO, scales="free_y")+
  guides(fill=guide_legend(nrow=3,keywidth = 0.6,keyheight = 0.6))
...
```{r}
#Add a month column
alldays<-seq(ymd(gdata::first(sort(globalcases$date))),
...
plot.seq.vs.case.per.month.div.voc
ggsave(paste(f.out,"/monthly.seq_per_case.bycountry.byVOC.png",sep=""),width=8.5, height=4.5, units="in")  
...
#FOR THE SUPP
plot.seq.vs.case.per.month.country.voc.cor<-
meta.glob.var.country.daily.full.month.temp %>%
  filter(!is.na(var.WHO))%>%
  ggplot(group=var.WHO)+
  geom_point(aes(x=totalVOCcases,y=totalVOCseqs,color=country))+
  # geom_smooth(aes(x=(totalVOCcases), y=(totalVOCseqs)),
              # method="glm",se=T,formula=y~x,lwd=0.2,alpha=0.3)+
  ...
  guides(color=guide_legend(title="Global Region",title.position = "left", 
                           keywidth = 0.6,keyheight = 0.6,nrow=3))
plot.seq.vs.case.per.month.country.voc.cor
ggsave(paste(f.out,"/monthly.seq_per_case.bycountry.byVOC.png",sep=""),width=8.5, height=5, units="in")  

#version wiht B)
plot_grid(plot.seq.vs.case.per.month.country.voc.cor,labels="B")
ggsave(paste(f.out,"/monthly.seq_per_case.bycountry.byVOC.B.png",sep=""),width=8.5, height=5, units="in")  
...
plot.seq.vs.case.per.month.country.voc.cor.log<-
meta.glob.var.country.daily.full.month.temp %>%
...
plot.seq.vs.case.per.month.country.voc.cor
ggsave(paste0(f.out,"/monthly.seq_per_case.bycountry.byVOC.log10.png"),width=8.5, height=5, units="in")  
...
## make a supplement of Canada seqs sampled per day and proportional months
```{r}
plot.daily.ave.seqs.prov.voc2<-plot.daily.ave.seqs.prov.voc+theme(axis.text.x=element_text(size=rel(1)))
# plot.monthly.proportion.byprov.VOC
plot_grid(plot.daily.ave.seqs.prov.voc2,plot.monthly.proportion.byprov.VOC,
          labels=LETTERS[1:2],nrow=2, rel_heights = c(1.2,1))
ggsave(paste0(f.out,"/figSX.CanSequencesSampled.png"),height=7,width=7)
```

## make a supplement of Global seqs sampled per day and proportional months
```{r}
plot_grid(plot.daily.ave.seqs.voc.country2, plot.monthly.proportion.bycountry.VOC,
          labels=LETTERS[1:2],nrow=2, rel_heights = c(1.3,1))
ggsave(paste0(f.out,"/figSX.GlobSequencesSampled.png"),height=8,width=8)
```


```{r}
## TEST
temp<-meta.glob.var.country.daily.full %>% 
  unite(col="temp",2:4,remove = F)
