...
#Modifiable inputs
focal.var<-"BA11"
Focal.var<-"BA.1.1" #str_to_title(focal.var)
...
per1<-as.Date(c("2021-11-26","2021-12-07"))
per2<-as.Date(c("2021-12-08","2021-12-18"))
...
sum.par.in<-paste0("../03_ancestral/2023-07-05_",focal.var,"_analysis/sum.par.roll.summary.csv")
sing.par.in<-paste0("../03_ancestral/2023-07-05_",focal.var,"_analysis/sing.par.roll.summary.csv")
...
p.focal.var.cases<-ggplot(focal.var.cases)+
  ...
  labs(x=NULL, y=paste0("Daily ave. ",Focal.var," cases in ",focal.source[2]))+
  #custom annot for diff locales
  annotate(geom="text",x =int.end-25,
           y=y.lim2[1]+2200, vjust=0.5, hjust=0.5,
           label=focal.source[1],size=2.5,
           color=s.afr.col)+
  annotate(geom="text",x =int.end+4,
           y=y.lim2[2]-4900, vjust=0.5, hjust=0.5,
...
## MERGE SOUTH AFRICA AND AFRICA: cases and sublins
```{r}
### NOTE THAT BA11 was detected from BOTH South Africa and Africa
# table(sum.Par.Roll.focal$Parent.Location)

## merge south africa and africa together
focal.var.cases.all<-focal.var.cases %>% dplyr::group_by(date) %>%
  dplyr::summarise(avgVOC_cases=sum(avgVOC_cases,na.rm=T))
focal.var.cases.all$country<-"Africa"

#replot
y.lim2<-c(0,(ceiling(max(focal.var.cases.all$avgVOC_cases)+2000)))
p.focal.var.cases.all<-ggplot(focal.var.cases.all)+
    annotate(geom="rect",xmin = as.Date(int.start), xmax = as.Date(int.end),
             ymin = y.lim2[1], ymax =y.lim2[2]-50, color="grey85",size=0,  fill="grey80")+
  annotate(geom="text",x =( as.Date(int.start)+int.duration/1.3),
           y=y.lim2[2]-500, vjust=1, hjust=0.5,
  ...
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=NULL, y=paste0("Daily ave. ",Focal.var," cases in all ",focal.source[2]))+
  annotate(geom="text",x =int.end-5,
           y=10000, vjust=0.5, hjust=0.5,
           label="All Africa",size=2.5,
           color=afr.col)
  
p.focal.var.cases.all
ggsave(paste0(f.out,"p.focal.var.cases.allafr.png"),height=3,width=3,unit="in")

### MERGE sum par roll
sum.Par.Roll.focal$date<-sum.Par.Roll.focal$tmrca.dt
sum.Par.Roll.focal.all<-sum.Par.Roll.focal %>% dplyr::group_by(date) %>%
  dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm = T),
                   lower.CI=sum(lower.CI,na.rm=T),
                   upper.CI=sum(upper.CI,na.rm=T))


## join the focal cases to the importation rate pre intervention
#this is limited to pre intervention...not anymore
# MERGED DATA 
focal.cases.intros<-left_join(sum.Par.Roll.focal.all, focal.var.cases.all, by=c("date"))
...
p.sublin<-ggplot(focal.cases.intros)+
  ...
  GlobFillScale+
  theme(legend.position = "none")+
  labs(x=NULL, y=paste0(Focal.var, " sublin./wk. from all ",focal.source[2]))
...
#now join this travel data onto focal cases intro (now merged for two locations, and trav data is for all africa, so this works)
focal.cases.intros$tmrca.dt<-focal.cases.intros$date
...
p.var.travel<-ggplot(focal.cases.intros.trav)+
  ...
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=NULL, y=paste0("Travellers per day from all ",focal.source[2]))+
  coord_cartesian(y=y.lim2)
  
p.var.travel
ggsave(paste0(f.out,"p.observedtravel.all.png"),height=3,width=3,unit="in")
...
focal.cases.intros.trav.long<-focal.cases.intros.trav %>% pivot_longer(cols = 8:9, 
...
#make grob of all 
plot_grid(p.focal.var.cases.all, p.var.travel2, p.sublin, labels=LETTERS[1:3], nrow=1)
...
#subset the data to when cases are higher
subset<-10
...
p.focal.cases.intros<-ggplot(focal.cases.intros.trav.pre,
  ...
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=paste0("Daily ave. ",Focal.var," cases in all ",focal.source[2]), 
       y=paste0(Focal.var," sublin./wk. from all ",focal.source[2]))+
...
#significantly imporved
BIC(lm.cases.intros.trav)
BIC(lm.cases.intros)
AIC(lm.cases.intros.trav)
AIC(lm.cases.intros)

#does support the addition of travel volume as a predictor
...
final.model<-lm.cases.intros.trav #WITH TRAVEL!!
...
focal.var.cases.trav.dur2$country<-focal.source[2]
...
p.focal.obs.pred<-ggplot(focal.obs.pred,
  ...
  labs(x=paste0("Daily ave. ",Focal.var," cases in ", focal.source[2]), 
       y=paste0(Focal.var, " sublin./wk. from all ",focal.source[2]))+
  pubTheme+
...
focal.var.cases.trav.dur3<-focal.var.cases.trav.dur2 %>%
  left_join(focal.var.cases.all[,c("date","avgVOC_cases")],by="avgVOC_cases")
focal.var.cases.trav.dur3$type<-"Predicted"
# head(focal.var.cases.trav.dur3)

focal.intros.all<-sum.Par.Roll.focal.all
...
p.focal.pred.sublin<-focal.intros.all %>%
  ...
  annotate(geom="text",x = (as.Date(int.start)+int.duration/1.5),
           ...
           label=int.descrip,fontface="italic",color="grey25",size=3)+
labs(x="Date of most recent common ancestor",y=paste0(paste0(Focal.var, " sublin./wk. from ",focal.source.brf)))+  scale_x_date(date_breaks = "1 month", date_labels = "%b %Y",
...
cases.averted.all.df.pre<-draw.averted.cases(N=averted.sublin,
                                             gamma.df=ans.pre)
#### NEW #####
ifelse(averted.sublin.per1<0, N1<-0, N1<-averted.sublin.per1)
cases.averted.all.df.pre.per1<-draw.averted.cases(N=N1,
                                             gamma.df=ans.pre)
ifelse(averted.sublin.per2<0, N2<-0, N2<-averted.sublin.per2)
cases.averted.all.df.pre.per2<-draw.averted.cases(N=N2,
                                             gamma.df=ans.pre)
...
#period 2
ifelse(averted.sublin.per2.lower<0, N1<-0, N1<-averted.sublin.per2.lower)
cases.averted.lower.per2<-draw.averted.cases(N=N1,
                                           gamma.df = ans.pre)
cases.averted.lower.ci.per2<-mean.95ci.X(cases.averted.lower.per2$cases.averted.all)

ifelse(averted.sublin.per2.upper<0, N2<-0, N2<-averted.sublin.per2.upper)
cases.averted.upper.per2<-draw.averted.cases(N=N2,
                                           gamma.df = ans.pre)
...
```{r}
plot_grid(p.focal.var.cases.all, p.focal.cases.intros, 
...
#horizontal version with different lettering 
plot_grid(p.focal.var.cases.all, p.focal.cases.intros, 
          p.focal.pred.sublin, p.averted1.varcol, nrow=1,labels=LETTERS[5:8],
          align="hv")
ggsave(paste0(f.out,"composite_casesAverted_full_h_efgh.png"),height=3.8,width=13,units = "in")
...
#singleton rate from focal sources
sing.Par.Roll.focal<-sing.Par.Roll.summary %>% 
  filter(par.state%in%focal.source) 
### MERGE AFRICA AND SOUTH AFR
sing.Par.Roll.focal$date<-sing.Par.Roll.focal$tmrca.dt.half
sing.Par.Roll.focal.all<-sing.Par.Roll.focal %>% dplyr::group_by(date) %>%
  dplyr::summarise(intros_meansum7d.mean=sum(intros_meansum7d.mean,na.rm = T),
                   lower.CI=sum(lower.CI,na.rm=T),
                   upper.CI=sum(upper.CI,na.rm=T))
sing.Par.Roll.focal.all$country<-focal.source[2]

sing.Par.Roll.focal.pre<-sing.Par.Roll.focal.all %>% 
  filter(date < (int.start)) 

## join the focal cases to the importation rate pre intervention
#MERGED
focal.cases.intros.sing<-left_join(sing.Par.Roll.focal.pre, focal.var.cases.all, by="date")
# head(focal.cases.intros.sing)
focal.cases.intros.sing$country<-focal.source[2]
# focal.cases.intros.sing<-focal.cases.intros.sing[which(focal.cases.intros.sing$country==focal.source[1]),]
focal.cases.intros.sing.res<-focal.cases.intros.sing %>% filter(avgVOC_cases>subset+15)
...
p.sing.focal.cases.intros.sing<-ggplot(focal.cases.intros.sing,
  ...
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=paste0("Daily ave. ",Focal.var," cases in ",focal.source[1]), 
       y=paste0(Focal.var, " single./wk. from ",focal.source[1]))+
...
p.sing.obs.fit<-ggplot(focal.var.cases.trav.dur.sing2,
  ...
  theme(legend.position = "none")+
  labs(x=paste0("Daily ave. ",Focal.var," cases in ",focal.source[2]), 
       y=paste0(Focal.var, " single./wk. from ",focal.source[2]))
p.sing.obs.fit
...
p.sing.focal.pred.singleton<-focal.intros.sing.all %>%
  ...
  annotate(geom="text",x = (as.Date(int.start)+int.duration/2),y=y.lim[2]-0.2, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
  labs(x="Date of most recent common ancestor",y=paste0("Singletons per week from ",focal.source[2]))+
  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0))+
  #observed
...
y.lim2<-c(0,(ceiling(max(can.var.cases$avgVOC_cases,na.rm=T))+8700))
...
p.can.var.cases<-ggplot(can.var.cases)+
    ...
    # GlobFillScale+
    theme(legend.position = "none")+
    labs(x=NULL, y=paste0("Daily ave. ",Focal.var," cases in Canada"))+
      annotate(geom="text",x =( as.Date(int.start)+int.duration/1.5),
