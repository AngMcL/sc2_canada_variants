...
focal.source<-c("South Africa")
focal.source.brf<-c("SouthAfrica")
int.yn<-TRUE
...
per1<-as.Date(c("2020-12-24","2021-01-21"))
per2<-as.Date(c("2021-01-22","2021-02-22"))
...
trav.in<-paste0("../07_StatCan_TravelData/Results_202305/ExpectedTravelVolume_Final_202305/trav.",
                tolower(focal.source.brf),"-",focal.var,".obs-exp.csv")
...
p.sublin<-ggplot(focal.cases.intros)+
  ...
  GlobFillScale+
  theme(legend.position = "none")+
  labs(x=NULL, y=paste0(Focal.var, " sublineages per week from ",focal.source))
...
sum.lm.cases.intros.trav<-summary(lm.cases.intros.trav) 
...
plot_grid(p.focal.var.cases, p.focal.cases.intros, 
          ...
          align="hv")
ggsave(paste0(f.out,"composite_casesAverted_full.png"),height=8,width=7,units = "in")
...
focal.cases.intros.sing.res<-focal.cases.intros.sing %>% filter(avgVOC_cases>(subset+1500))
...
p.sing.focal.pred.singleton<-focal.intros.sing.all %>%
  ...
  annotate(geom="text",x = (as.Date(int.start)+int.duration/2),y=y.lim[2]-0.2, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
labs(x="Date of most recent common ancestor",y=paste0(paste0(Focal.var, " single./wk. from ",focal.source.brf)))+  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0))+
  #observed
...
y.lim2<-c(0,(ceiling(max(can.var.cases$avgVOC_cases,na.rm=T))+150))
var.col<-varPalette.ch[which(names(varPalette.ch)==Focal.var)]
