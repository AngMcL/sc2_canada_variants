...
per1<-as.Date(c(int.start,"2021-12-07"))
per2<-as.Date(c("2021-12-08",int.end))
...
sum.par.in<-paste0("../03_ancestral/2023-02-10_",focal.var,"_analysis/sum.par.roll.summary.csv")
sing.par.in<-paste0("../03_ancestral/2023-02-10_",focal.var,"_analysis/sing.par.roll.summary.csv")
...
p.focal.var.cases<-ggplot(focal.var.cases)+
  ...
  labs(x=NULL, y=paste0("Average daily ",Focal.var," cases in ",focal.source))
...
focal.cases.intros.trav.pre.res<-focal.cases.intros.trav.pre %>% filter(avgVOC_cases>6000)
...
p.focal.cases.intros<-ggplot(focal.cases.intros.trav.pre,
  ...
  GlobColScale+
  theme(legend.position = "none")+
  labs(x=paste0("Average daily ",Focal.var," cases in ",focal.source), 
...
focal.cases.intros.sing.res<-focal.cases.intros.sing %>% filter(avgVOC_cases>7500)
...
p.sing.focal.obs.pred<-ggplot(focal.obs.pred,
  ...
  labs(x=paste0("Average daily ",Focal.var," cases in ",focal.source), y="Singletons per week")+
