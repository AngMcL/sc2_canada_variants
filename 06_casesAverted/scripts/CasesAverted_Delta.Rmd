...
focal.source<-c("India")
focal.source.brf<-c("India")
int.yn<-TRUE
...
per1<-as.Date(c("2021-04-22","2021-06-30"))
per2<-as.Date(c("2021-07-01","2021-09-26"))
int.duration.per1<-as.numeric(diff(per1)) 
int.duration.per2<-as.numeric(diff(per2)) 
#for figures
fig.start.date<-as.Date("2021-02-01")
...
trav.in<-paste0("../07_StatCan_TravelData/Results_202305/ExpectedTravelVolume_Final_202305/trav.",
                tolower(focal.source),".obs-exp.csv")
...
scaleDateFlexMore<-scale_x_date(date_labels = "%b %Y",expand=c(0,0),
                                limits=c(fig.start.date,fig.end.date+120))
...
#for delta, restrict further for fit
subset<-"2021-04-11"
cat(paste0("Models trained on data subsetted to date greater than ",subset),
    file=text.out,sep="\n",append=T)

focal.cases.intros.trav.pre.res<-focal.cases.intros.trav.pre %>%
  filter(date < int.start & date > as.Date(subset) )
...
## LINEAR MODELS
#with log transformed cases (improves the R2)
...
p.focal.cases.intros<-ggplot(focal.cases.intros.trav.pre,
  ...
  geom_smooth(data=focal.cases.intros.trav.pre.res,
...
lm.cases.intros.trav<-lm(data=focal.cases.intros.trav.pre, 
...
## sublin averted each day
```{r}
#mutate a column that is daily sublin averted=predicted - observed
# focal.var.cases.trav.dur3 #pred
# focal.intros.all #obs

daily.averted<-left_join(focal.var.cases.trav.dur3 ,focal.intros.all, by="date")
# head(daily.averted)

daily.averted<-daily.averted %>% mutate(daily.averted.sublin=intros_meansum7d.mean.x - intros_meansum7d.mean.y )
ggplot(daily.averted)+
  geom_point(aes(x=date,y=daily.averted.sublin))

#compare to daily travellers averted
trav.vol.red2<-trav.vol.red
colnames(trav.vol.red2)[1]<-"date"
trav.vol.red2<- trav.vol.red2 %>% mutate(daily.trav.averted = `Expected travel` - `Observed travel`)
# trav.vol.red2$date<-as.Date(trav.vol.red2$date)
daily.averted.trav<-left_join(daily.averted, trav.vol.red2,by="date")

#are they correlated?
cor(daily.averted.trav$daily.averted.sublin, daily.averted.trav$daily.trav.averted)

ggplot(daily.averted.trav)+
  geom_point(aes(x=daily.trav.averted, y=daily.averted.sublin))
...
sing.Par.Roll.focal.pre$date<-sing.Par.Roll.focal.pre$tmrca.dt.half+14
# class(focal.var.cases$tmrca.dt)
...
pred.pal2<-pred.pal
names(pred.pal2)<-c("Observed","Predicted")
y.lim<-c(0,max(focal.var.cases.trav.dur.sing3$intros_meansum7d.mean)+14)
p.sing.focal.pred.singleton<-focal.intros.sing.all %>%
  ...
  annotate(geom="text",x = (as.Date(int.start)+int.duration/2),y=y.lim[2]-0.2, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=4)+
 labs(x="Date of most recent common ancestor",
      y=paste0(paste0(Focal.var, " single./wk. from ",focal.source.brf)))+  scale_x_date(date_breaks="1 month", date_labels = "%b %Y",
                            limits=as.Date(c("2021-01-01",data.date)),
                            expand=c(0,0))+
  scale_y_continuous(expand=c(0,0))+
  #observed
   geom_line(aes(x=date+14,y=intros_meansum7d.mean,color=type),
               stat="identity",size=1.5,alpha=0.7)+
      geom_ribbon(aes(x=date+14,ymin=lower.CI,ymax=upper.CI,fill=type),
               stat="identity",alpha=0.5)+
  #predicted
