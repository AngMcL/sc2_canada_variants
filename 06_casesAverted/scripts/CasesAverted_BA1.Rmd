...
#Modifiable inputs
focal.var<-"BA1"
Focal.var<-"BA.1" #str_to_title(focal.var)
...
p.focal.var.cases<-ggplot(focal.var.cases)+
  ...
  annotate(geom="text",x =int.end-20,
           y=y.lim2[2]-6000, vjust=0.5, hjust=0.5,
           label=focal.source[1],size=2.5,
           color=s.afr.col)+
  annotate(geom="text",x =int.end+35,
...
### NOTE THAT BA1 was detected from South Africa but not Africa
table(sum.Par.Roll.focal$Parent.Location)

#restrict this to also be s africa only
focal.var.cases<-focal.var.cases[which(focal.var.cases$country=="South Africa"),]
...
#subset to S africa (only source of BA.1, none from Africa)
...
p.obs.fit<-ggplot(focal.var.cases.trav.dur2,
  ...
  GlobFillScale+
  theme(legend.position = "none")+
  labs(x="Daily ave. travel volume",
...
p.sing.focal.cases.intros.sing<-ggplot(focal.cases.intros.sing,
  ...
  theme(legend.position = "none")+
  labs(x=paste0("Daily ave. ",Focal.var," cases in ",focal.source[1]), 
       y=paste0(Focal.var, " single./wk. from ",focal.source.brf))+
...
p.sing.focal.pred.singleton<-focal.intros.sing.all %>%
  ...
  annotate(geom="text",x = (as.Date(int.start)+int.duration/2),y=y.lim[2]-0.2, vjust=1, hjust=0.5,
           label=int.descrip,fontface="italic",color="grey25",size=3)+
  labs(x="Date of most recent common ancestor",
       y=paste0("Singletons per week from ",focal.source[1]))+
  scaleDateFlexLess+
  scale_y_continuous(expand=c(0,0))+
  #observed
